    ...
    def _get_data(self, query_type, useCache):
        ...
        if useCache:
            ...
            if sys.version_info.major == 3:# Python 3
                tmpCheck = self.WINDOW.getProperty(query_type + "-data") != ""
            else:# Python 2
                tmpCheck = self.WINDOW.getProperty(query_type + "-data") is not ""
            if tmpCheck:
        ...
        def query_recommended_episodes():
            ...
            if "result" in json_query and 'tvshows' in json_query['result']:
                for item in json_query['result']['tvshows']:
                    if sys.version_info.major == 3:# Python 3
                        checkAbortRequested = xbmc.Monitor().abortRequested()
                    else:# Python 2
                        checkAbortRequested = xbmc.abortRequested
                    if checkAbortRequested:
                        break
