...
def calc_sub_percent_sync(sub_filename, array_original):
    try:
        ...
        #fix for 'Opensubtitles" subs - remo
            ...
            if len(sub_release_type) > 0 and len(video_release_type) > 0 and sub_release_type[-1] == video_release_type[-1]:
                ...
                if len(video_quality) > 0 and len(sub_quality) > 0 and sub_quality[-1] == video_quality[-1]:
                    for i in range(1):
                        ...
                        #log(__name__, "Video source quality: %s" %repr(video_quality[0]))
                        #log(__name__, "Subtitle quality: %s" %repr(sub_quality[0]))

        #log(__name__, "Video source array for compare: %s" %array_original)
        #log(__name__, "Subtitle array for compare: %s" %array_subs)
        precent = similar(array_original,array_subs)
        return precent

    except Exception as e:
        log(__name__, "Error in calc_sub_percent_sync: " + repr(e) + " | sub_filename: " + repr(sub_filename))
        return 0
...
def build_subs_list_with_percentage(json, item):
    ...
    if json != 0:
        for item_data in json:
            sub = {}
            sub['title'] = item_data["SubFileName"]
            sub['LanguageName'] = item_data['LanguageName']
            sub["ISO639"] = item_data["ISO639"]
            sub["SubHearingImpaired"] = item_data["SubHearingImpaired"]
            sub["ZipDownloadLink"] = item_data["ZipDownloadLink"]
            sub["IDSubtitleFile"] = item_data["IDSubtitleFile"]
            sub["SubFileName"] = item_data["SubFileName"]
            sub["SubFormat"] = item_data["SubFormat"]
