...
class BuildMenu:
    ...
    def view_build(self, name):
        ...
        for version, url, gui, kodi, themefile, icon, fanart, preview, adult, info, description in match:
            ...
            directory.add_separator()
            directory.add_dir('Save Data Menu', {'mode': 'savedata'}, icon=CONFIG.ICONSAVE, themeit=CONFIG.THEME2)
            ...
            if guicheck:
                directory.add_file('Apply build fix', {'mode': 'install', 'action': 'gui', 'name': name}, description=description, fanart=fanart,
                                   icon=icon, themeit=CONFIG.THEME1)
