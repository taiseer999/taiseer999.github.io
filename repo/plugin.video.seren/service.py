...
try:
    ...
    g.wait_for_abort(5)  # Sleep for a half a minute to allow widget loads to complete.
    while not monitor.abortRequested():
        xbmc.executebuiltin(
            'RunPlugin("plugin://plugin.video.seren/?action=runMaintenance")'
        )
        if not g.wait_for_abort(5):  # Sleep to make sure tokens refreshed during maintenance
            xbmc.executebuiltin(
                'RunPlugin("plugin://plugin.video.seren/?action=syncTraktActivities")'
            )
        if not g.wait_for_abort(5):  # Sleep to make sure we don't possibly clobber settings
            xbmc.executebuiltin(
        ...
        if g.wait_for_abort(7 * randint(13, 17)):
            break
finally:
    del monitor
    g.deinit()
