...
class Plex:  # pylint: disable=too-many-public-methods, too-many-instance-attributes
    ...
    def __init__(self, settings, load=False):
        ...
        self.discover_server = 'https://discover.provider.plex.tv'
    ...
    def talk_to_discover(self, path, params=None):
        params = params or {}
        full_url = '%s%s' % (self.discover_server, path)
        LOG.debug('discover url = %s' % full_url)
        try:
            ...
            LOG.error('Discover endpoint unreachable: %s' % error)
            return '<?xml version="1.0" encoding="UTF-8"?><message status="error"></message>'
        except requests.exceptions.ReadTimeout:
            LOG.debug('Discover read timeout for %s' % full_url)
            return '<?xml version="1.0" encoding="UTF-8"?><message status="error"></message>'

        LOG.debugplus('Full Discover URL was: %s' % response.url)
        LOG.debugplus('Full Discover header sent was: %s' % response.request.headers)
        LOG.debugplus('Full Discover header received was: %s' % response.headers)

        if response.status_code >= 400:
            LOG.error('Discover HTTP response error: %s' % response.status_code)
        ...
        LOG.debugplus('Discover XML: \n%s' % link)
        return link

    def get_processed_discover_xml(self, path, params=None):
        data = self.talk_to_discover(path, params=params)
        return ETree.fromstring(data)
