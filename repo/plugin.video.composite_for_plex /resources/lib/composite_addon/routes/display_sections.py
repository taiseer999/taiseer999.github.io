...
def run(context, content_filter=None, display_shared=False):
    ...
    # 1. الصفحة الرئيسية السريعة لـ DPlex
    items += dplex_home_items(context)

    # 2. إضافة قائمة Watchlist السحابية في الصفحة الرئيسية
    if context.plex_network.is_myplex_signedin():
        details = {'title': i18n('Watchlist')}
        extra_data = {
            'type': 'Folder',
            'mode': MODES.MYPLEXWATCHLIST
        }
        gui_item = GUIItem('http://myplexwatchlist', details, extra_data)
        items.append(create_gui_item(context, gui_item))

    # 3. إضافة الأقسام المدمجة
    if server_section_menus:
        items += combined_sections_item(context)

    # 4. إضافة أقسام السيرفرات العادية
...
def dplex_home_items(context):
    items = []
    menu_structure = [
        (i18n('Continue Watching TV'), MODES.CONTINUE_WATCHING_TV, 'http://combined/continueWatchingTv', None),
        (i18n('Continue Watching Movies'), MODES.MOVIES_ON_DECK, 'http://combined/continueWatchingMovies', 'movie'),
        (i18n('Next Up TV'), MODES.NEXT_UP_TV, 'http://combined/nextUpTv', None),
    ]

    for title, mode, url, content_type in menu_structure:
        details = {'title': title}
        extra_data = {'type': 'Folder', 'mode': mode}
        if content_type:
