...
class AddonSettings:  # pylint: disable=too-many-public-methods
    ...
    def _coerce_limit(value, default):
        ...
        if not text:
            return default
        if text in ('0', 'unlimited', 'all', 'max', 'none'):
            return None
    ...
    def performance_home_limit(self):
        return self._coerce_limit(self._get_setting('perf_home_limit'), 12)

    def performance_page_size(self):
        return self._coerce_limit(self._get_setting('perf_page_size'), 40)

    def performance_light_requests(self):
        value = self._get_setting('perf_light_requests')
