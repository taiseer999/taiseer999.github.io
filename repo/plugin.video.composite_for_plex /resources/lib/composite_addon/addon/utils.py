...
def write_pickled(filename, data):
    ...
    temp_filename = ''.join([filename, '.tmp'])
    pickled_data = pickle.dumps(data, protocol=2)

    with open(temp_filename, 'wb') as open_file:
...
def read_pickled(filename, delete_after=True):
    ...
    except (OSError, EOFError, ValueError, TypeError, pickle.PickleError, AttributeError):
        LOG.debug('Unable to read pickled file: %s' % filename)
        LOG.debug('Treating pickled data as missing')
        try:
            os.remove(filename)
        except OSError:
            pass
