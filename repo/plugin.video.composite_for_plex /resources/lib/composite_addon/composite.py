...
def run(start_time):  # pylint: disable=too-many-locals, too-many-statements, too-many-branches, too-many-return-statements
    ...
    if mode == MODES.MYPLEXWATCHLIST:
        from .routes import myplex_watchlist
        myplex_watchlist.run(context)
        return _finished(start_time)

    if mode == MODES.CONTINUE_WATCHING_TV:
        from .routes import continue_watching_tv
        continue_watching_tv.run(context)
        return _finished(start_time)

    if mode == MODES.NEXT_UP_TV:
        from .routes import next_up_tv
        next_up_tv.run(context)
        return _finished(start_time)
