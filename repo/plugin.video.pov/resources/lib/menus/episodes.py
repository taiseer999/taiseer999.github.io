...
class Episodes:
	...
	def build_episode_content(self, _position, ep_data):
		try:
			...
			if self.watched_indicators == 1: cm_append((self.cm_sort['trakt'], traktmanager_str, run_plugin % build_url({
				'mode': 'trakt_manager_choice', 'media_type': 'tvshow', 'tmdb_id': tmdb_id, 'imdb_id': imdb_id, 'tvdb_id': tvdb_id
			})))
			if self.watched_indicators == 2: cm_append((self.cm_sort['mdblist'], mdblmanager_str, run_plugin % build_url({
				'mode': 'mdbl_manager_choice', 'media_type': 'tvshow', 'tmdb_id': tmdb_id, 'imdb_id': imdb_id, 'tvdb_id': tvdb_id
			})))
