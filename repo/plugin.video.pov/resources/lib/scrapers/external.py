	...
	def process_torrents(self, torrent_sources):
		...
		try:
			...
			for item in self.debrid_torrents:
				...
				if self.display_uncached_torrents: torrent_results += [{**i, 'cache_provider': 'Uncached %s' % item} for i in torrent_sources if i['provider'] != 'debridio' and not i['hash'] in _hashes]
