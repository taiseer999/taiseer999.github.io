...
#		self.meta = meta
		...
		for item in debrid_enabled:
			i = Thread(target=debrid_runners[item], name=item)
			main_threads_append(i)
			i.start()
		if background: [i.join() for i in self.main_threads]
		else: self.debrid_check_dialog()
		for i in self.hashes_to_cache: debrid_cache.set_many(i[0], i[1])
		return {
	...
	def RD_check(self):
		...
		threads = list(make_thread_list(_process, list(chunks(unchecked_hashes, MAX_CHUNKS)), Thread))
		[i.join() for i in threads]
#		rd_cache = rd_api.check_cache(unchecked_hashes)
	...
	def debrid_check_dialog(self):
		...
		len_main_threads = len(self.main_threads)
		while not self.progress_dialog.iscanceled() or monitor.abortRequested():
			try:
				...
				line = '%s[CR]%s[CR]%s' % (plswait_str, checking_debrid_str, insert_line)
				progress = int((len_main_threads-len(remaining_debrids))/len_main_threads*100)
				self.progress_dialog.update(line, progress)
				sleep(self.sleep_time)
				if self.processing_hashes:
