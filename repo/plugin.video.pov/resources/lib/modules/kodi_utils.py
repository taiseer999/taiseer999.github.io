...
class ListObj:
	fanart = get_addoninfo('fanart') #translate_path('special://home/addons/plugin.video.pov/fanart.png')
	poster = translate_path('special://home/addons/plugin.video.pov/resources/media/box_office.png')
	item_jump = translate_path('special://home/addons/plugin.video.pov/resources/media/item_jump.png')
	item_next = translate_path('special://home/addons/plugin.video.pov/resources/media/item_next.png')
	run_plugin, container_refresh, container_update = 'RunPlugin(%s)', 'Container.Refresh(%s)', 'Container.Update(%s)'
	nextpage_str, jumpto_str = local_string(32799), local_string(32964)

	def __init__(self):
		import sys
		self.handle = int(sys.argv[1])
		self.KODI_VERSION = get_kodi_version()
		self.view_type, self.content_type = 'view.main', ''
		self.sort_type, self.category = 'unsorted', ''
		self.items = []
		self.make_listitem = make_listitem
		self.build_url = build_url

	def end(self, cacheToDisc=None):
		if self.items: add_items(self.handle, self.items)
		if self.category: set_category(self.handle, self.category)
		if self.sort_type: set_sort_method(self.handle, self.sort_type)
		set_content(self.handle, self.content_type)
		end_directory(self.handle, cacheToDisc=cacheToDisc)
		set_view_mode(self.view_type, self.content_type)

	def add_dir(self, url_params, list_name, iconImage=None, fanartImage=None, isFolder=True):
		if 'new_page' in url_params: list_name = f"{list_name} >> {url_params['new_page']} <<"
		fanart = fanartImage or self.fanart
		icon = iconImage or self.item_next
		url = build_url(url_params)
		listitem = make_listitem()
		listitem.setLabel(list_name)
		listitem.setArt({'icon': icon, 'poster': icon, 'thumb': icon, 'fanart': fanart, 'banner': icon})
