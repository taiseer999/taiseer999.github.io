...
from modules import kodi_utils, settings, infotagger
...
class POVPlayer(kodi_utils.xbmc_player):
	...
	def run(self, url=None, media_type=None):
		...
		try:
			...
			background = self.meta.get('background', False) is True
			...
			try:
				...
				if self.media_type == 'movie':
					if KODI_VERSION < 20:
						...
						infotagger.infoTagger(videoinfo, self.meta)
	...
	def monitor(self):
		...
		while self.isPlayingVideo():
			try:
				...
				if self.autoplay_next_episode or self.autoscrape_nextep:
					...
					if self.remaining_time <= self.start_prep:
						...
						if not self.nextep_started and self.autoscrape_nextep:
							if not self.autoplay_nextep:
								self.autoscrape_next_episode = True
			except: pass
