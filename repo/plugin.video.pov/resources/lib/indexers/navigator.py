...
class Navigator:
	...
	def main(self):
		...
		add_items(__handle__, list(self.build_main_lists()))
		self._end_directory()
	...
	def offcloud(self):
		...
		clca_str, n_ins = ls(32497) % oc_str, _in_str % (oc_str.upper(), '%s')
		self.AD({'mode': 'offcloud.oc_torrent_cloud'       }, n_ins % cloud_str, 'offcloud.png')
		self.AD({'mode': 'offcloud.oc_account_info'        }, n_ins % ai_str   , 'offcloud.png', False)
		self.AD({'mode': 'offcloud.user_cloud_clear'       }, n_ins % clc_str  , 'offcloud.png', False)
		self.AD({'mode': 'clear_cache', 'cache': 'oc_cloud'}, n_ins % clca_str , 'offcloud.png', False)
	...
	def my_content(self):
		...
		self.AD({'mode': 'build_trakt_list.get_trakt_trending_popular_lists', 'list_type': 'trending'}, n_ins % tu_str     , 'trakt.png')
		self.AD({'mode': 'build_trakt_list.get_trakt_trending_popular_lists', 'list_type': 'popular' }, n_ins % pu_str     , 'trakt.png')
		self.AD({'mode': 'build_trakt_list.search_trakt_lists'                                       }, n_ins % sea_str    , 'trakt.png')
		if trakt_status:
			...
			self.AD({'mode': 'trakt.trakt_account_info'                                              }, t_n_ins % ai_str   , 'trakt.png', False)
		if imdb_status:
	...
	def trakt_lists(self):
		...
		self.AD({'mode': 'build_trakt_list.get_trakt_lists', 'list_type': 'my_lists'   }, n_ins % ml_str , 'trakt.png')
		self.AD({'mode': 'build_trakt_list.get_trakt_lists', 'list_type': 'liked_lists'}, n_ins % ll_str , 'trakt.png')
	...
	def settings(self):
		...
		self.AD({'mode': 'open_settings'                }, n_ins % pov_str                 , 'pov.png', False)
		self.AD({'mode': 'open_settings', 'query': '6.0'}, n_ins % ms_str                  , 'settings.png' , False)
	...
	def clear_info(self):
		...
		self.AD({'mode': 'clear_all_cache',                         }, n_ins % clear_all      , 'tools.png', False)
		...
		self.AD({'mode': 'clear_cache', 'cache': 'oc_cloud'         }, n_ins % clear_oc_str   , 'tools.png', False)
		self._end_directory()
	...
	def log_utils(self):
		pov_vstr, log_path, kl_loc = k.addon().getAddonInfo('version'), 'special://home/addons/%s/changelog.txt', tp('special://logpath/kodi.log')
		pov_str, cl_str, lut_str, k_str, lv_str, lu_str = ls(32036), ls(32508), ls(32777), ls(32538), ls(32509), ls(32853)
		mt_str, mh_str = tp(log_path % 'plugin.video.pov'), '[B]%s[/B]: %s  [I](v.%s)[/I]' % (cl_str.upper(), pov_str, pov_vstr)
