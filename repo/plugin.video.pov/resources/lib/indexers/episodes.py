...
string, ls, make_cast_list = str, kodi_utils.local_string, kodi_utils.make_cast_list
...
class Episodes(kodi_utils.ListObj):
	...
	def run(self):
		try:
			...
			self.view_type, self.content_type = 'view.episodes', 'episodes'
		...
		self.category = ls(params_get('name'))
		self.end(False)
	...
	def build_episode_content(self, item_position, ep_data):
		try:
			...
			else: props = {'pov_sort_order': string(ep_data_get('sort', item_position))}
			cm = []
			cm_append = cm.append
			...
			show_poster = meta_get(self.poster_main) or meta_get(self.poster_backup) or self.poster
			fanart = meta_get(self.fanart_main) or meta_get(self.fanart_backup) or self.fanart
			...
			extras_params = self.build_url({'mode': 'extras_menu_choice', 'media_type': 'tvshow', 'tmdb_id': tmdb_id, 'is_widget': self.is_widget})
			options_params = self.build_url({'mode': 'options_menu_choice', 'content': 'episode', 'tmdb_id': tmdb_id, 'season': season, 'episode': episode})
			url_params = self.build_url({'mode': 'play_media', 'media_type': 'episode', 'tmdb_id': tmdb_id, 'season': season, 'episode': episode})
			if self.show_all_episodes:
				if self.all_episodes == 1 and meta_get('total_seasons') > 1: browse_params = self.build_url({'mode': 'build_season_list', 'tmdb_id': tmdb_id})
				else: browse_params = self.build_url({'mode': 'build_episode_list', 'tmdb_id': tmdb_id, 'season': 'all'})
			else: browse_params = self.build_url({'mode': 'build_season_list', 'tmdb_id': tmdb_id})
			browse_seas_params = self.build_url({'mode': 'build_episode_list', 'tmdb_id': tmdb_id, 'season': season})
			cm_append((options_str, self.run_plugin % options_params))
			cm_append((extras_str, self.run_plugin % extras_params))
			...
			if not unaired:
				if progress != '0' or resumetime != '0':
					clearprog_params = self.build_url({'mode': 'watched_unwatched_erase_bookmark', 'media_type': 'episode', 'tmdb_id': tmdb_id,
												'season': season, 'episode': episode, 'refresh': 'true'})
					cm_append((clearprog_str, self.run_plugin % clearprog_params))
					props['pov_in_progress'] = 'true'
				if playcount:
					if self.hide_watched: return
					unwatched_params = self.build_url({'mode': 'mark_as_watched_unwatched_episode', 'action': 'mark_as_unwatched', 'tmdb_id': tmdb_id,
												'tvdb_id': tvdb_id, 'season': season, 'episode': episode,  'title': title, 'year': year})
					cm_append((unwatched_str % self.watched_title, self.run_plugin % unwatched_params))
				else:
					watched_params = self.build_url({'mode': 'mark_as_watched_unwatched_episode', 'action': 'mark_as_watched', 'tmdb_id': tmdb_id,
												'tvdb_id': tvdb_id, 'season': season, 'episode': episode,  'title': title, 'year': year})
					cm_append((watched_str % self.watched_title, self.run_plugin % watched_params))
			if self.list_type == 'next_episode_trakt':
				cm_append((nextep_manager_str, self.container_update % self.build_url({'mode': 'build_next_episode_manager'})))
...
#			listitem.setContentLookup(False)
			...
			if self.KODI_VERSION < 20:
				...
				listitem.setProperty('watchedprogress', progress) if int(progress) else ...
	...
	def worker(self):
		...
		if self.is_widget: self.container_update = 'ActivateWindow(Videos,%s,return)'
		if self.list_type_starts_with('next_episode'):
		...
		if self.list_type_starts_with('trakt_dict'):
			pass
		elif self.list_type_starts_with('next_episode'):
			def func(function):
