...
from modules.utils import make_thread_list, paginate_list, sort_for_article, title_key, jsondate_to_datetime
...
def mdbl_modify_list(list_id, data, action='add'):
	if list_id: url = 'lists/%s/items/%s' % (list_id, action)
	else: url = 'watchlist/items/%s' % action
	result = call_mdblist(url, json=data, method='post')
	if 'detail' in result: kodi_utils.notification(result['detail'])
...
def mdbl_list_items(list_id, list_type):
	...
	elif list_id: url = 'lists/%s/items?unified=true' % list_id
	else: url = 'watchlist/items?unified=true'
...
def mdblist_watchlist(media_type, page_no, letter):
	...
	url = 'watchlist/items'
	original_list = mdbl_cache.cache_mdbl_object(call_mdblist, string, url)
...
def mdbl_indicators_movies(watched_info):
	...
	mdbl_cache.MDBLWatched().set_bulk_movie_watched(insert_list)
...
def mdbl_indicators_tv(watched_info):
	...
	threads = list(make_thread_list(_process, watched_items, Thread))
	[i.join() for i in threads]
	mdbl_cache.MDBLWatched().set_bulk_tvshow_watched(insert_list)
...
def mdbl_progress_movies(progress_info):
	...
	[i.join() for i in threads]
	mdbl_cache.MDBLWatched().set_bulk_movie_progress(insert_list)
...
def mdbl_progress_tv(progress_info):
	...
	threads = list(make_thread_list(_process, progress_items, Thread))
	[i.join() for i in threads]
	mdbl_cache.MDBLWatched().set_bulk_tvshow_progress(insert_list)
