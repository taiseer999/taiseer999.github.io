	...
	def _builder():
		for count, item in enumerate(files, 1):
			try:
				...
				size = float(int(item['size']))/1073741824
				display = '%02d | %.2f GB | [I]%s [/I]'
				if item['owned']: display = display % (count, size, '[COLOR cyan]%s[/COLOR]' % name)
				elif item['cached']: display = display % (count, size, '[COLOR magenta]%s[/COLOR]' % name)
				else: display = display % (count, size, name)
