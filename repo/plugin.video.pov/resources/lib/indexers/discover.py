	...
	def _set_default_params(self, media_type):
		if not 'media_type' in self.discover_params:
			...
			self.discover_params['search_string']['base'] = (
				'https://api.themoviedb.org/3/discover/%s?api_key=%s&language=en-US&page=%s' % (url_media_type, self.tmdb_api, '%s')
				)
			self.discover_params['search_string']['base_similar'] = (
				'https://api.themoviedb.org/3/%s/%s/similar?api_key=%s&language=en-US&page=%s' % (url_media_type, '%s', self.tmdb_api, '%s')
				)
			self.discover_params['search_string']['base_recommended'] = (
				'https://api.themoviedb.org/3/%s/%s/recommendations?api_key=%s&language=en-US&page=%s' % (url_media_type, '%s', self.tmdb_api, '%s')
				)
