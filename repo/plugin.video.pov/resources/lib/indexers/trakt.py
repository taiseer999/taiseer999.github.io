	...
	def _process():
		for item in lists:
			try:
				...
				url = build_url({'mode': 'build_trakt_list', 'user': user, 'slug': slug, 'list_id': list_id, 'list_type': 'user_lists'})
				cm_append((add2menu_str, 'RunPlugin(%s)' % build_url({'mode': 'menu_editor.add_external', 'name': name, 'iconImage': 'trakt.png'})))
				cm_append((add2folder_str, 'RunPlugin(%s)' % build_url({'mode': 'menu_editor.shortcut_folder_add_item', 'name': name, 'iconImage': 'trakt.png'})))
				cm_append((likelist_str, 'RunPlugin(%s)' % build_url({'mode': 'trakt.trakt_like_a_list', 'user': user, 'list_slug': slug})))
				cm_append((unlikelist_str, 'RunPlugin(%s)' % build_url({'mode': 'trakt.trakt_unlike_a_list', 'user': user, 'list_slug': slug})))
...
def build_trakt_list(params):
	def _process(item_type, items, func):
		if   item_type == 'movies':
			func(Movies({'id_type': 'trakt_dict', 'list': items}).worker())
		elif item_type == 'tvshows':
			func(TVShows({'id_type': 'trakt_dict', 'list': items}).worker())
	...
	user, slug = params.get('user'), params.get('slug')
	...
	for position, i in enumerate(process_list, 1):
		itype = i['type']
		if   itype == 'movie':
			item = {**i[itype]['ids'], 'sort': position}
			process_dict['movies'] += [item]
		elif itype == 'show':
			item = {**i[itype]['ids'], 'sort': position}
			process_dict['tvshows'] += [item]
		elif itype == 'season':
			item = {'tmdb_id': i['show']['ids']['tmdb'], 'season': i['season']['number'], 'sort': position}
			process_dict['seasons'] += [item]
		elif itype == 'episode':
			item = {'media_ids': {'tmdb': i['show']['ids']['tmdb']}, 'season': i['episode']['season'], 'episode': i['episode']['number'], 'sort': position}
			process_dict['episodes'] += [item]
	content, total = max(process_dict.items(), key=lambda x: len(x[1]))
	item_list = []
	item_list_extend = item_list.extend
	threads = []
	threads += [Thread(target=_process, args=('movies', process_dict['movies'], item_list_extend))]
	threads += [Thread(target=_process, args=('tvshows', process_dict['tvshows'], item_list_extend))]
	threads += [Thread(target=_process, args=('episodes', process_dict['episodes'], item_list_extend))]
	threads += [Thread(target=_process, args=('seasons', i, item_list_extend)) for i in process_dict['seasons']]
	[i.start() for i in threads]
	[i.join() for i in threads]
	item_list.sort(key=lambda k: int(k[1].getProperty('pov_sort_order')))
	__handle__ = int(sys.argv[1])
	if total_pages > 2 and not is_widget:
		url = {'mode': 'build_navigate_to_page', 'transfer_mode': 'build_trakt_list', 'media_type': 'Media',
	...
	if total_pages > page:
		page += 1
		url = {'mode': 'build_trakt_list', 'user': user, 'slug': slug, 'list_id': list_id, 'new_page': page, 'new_letter': letter, 'list_type': list_type}
		kodi_utils.add_dir(__handle__, url, nextpage_str)
