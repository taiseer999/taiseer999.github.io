...
trakt_main, trakt_personal = ('trakt_tv_trending', 'trakt_tv_trending_recent', 'trakt_tv_most_watched'), ('trakt_collection', 'trakt_watchlist', 'trakt_collection_lists')
personal, imdb_personal = ('in_progress_tvshows', 'favourites_tvshows', 'watched_tvshows'), ('imdb_watchlist', 'imdb_user_list_contents', 'imdb_keywords_list_contents')
...
personal_dict = {'in_progress_tvshows': ('caches.watched_cache', 'get_in_progress_tvshows'), 'favourites_tvshows': ('caches.favourites_cache', 'retrieve_favourites'),
...
class TVShows:
	...
	def build_tvshow_content(self, item_position, _id):
		try:
			...
			sort = _id.get('sort', item_position) if self.id_type == 'trakt_dict' else item_position
			...
			if self.is_widget: props.update({
				...
				'pov_random_params': random_params})
...
#			listitem.setContentLookup(False)
			...
			if KODI_VERSION < 20:
				...
				listitem.setProperty('watchedepisodes', string(total_watched)) if total_watched > 0 else ...
				listitem.setProperty('watchedprogress', string(int(total_watched / total_aired_eps * 100))) if total_aired_eps > 0 else ...
