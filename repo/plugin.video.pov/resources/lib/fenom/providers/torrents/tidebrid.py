...
class source:
	...
	def __init__(self):
		...
		params = f"/debridoptions=nodownloadlinks|{debrid}={token}" if token else ''
		self.language = ['en']
	...
	def sources(self, data, hostDict):
		...
		for file in files:
			try:
				...
				cached = 'TI+' if re.search(r'(?:RD\+|AD\+)', file['name']) else False
				# try:
				...
				item = {
					...
					'provider': cached or 'tidebrid', 'url': url, 'hash': hash, 'name': name, 'name_info': name_info,
