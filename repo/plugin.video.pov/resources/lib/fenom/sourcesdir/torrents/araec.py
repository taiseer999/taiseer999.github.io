...
class source:
	...
	def __init__(self):
		self.language = ['en']
		self.base_link = "https://torrentio.araec.com"
		self.movieSearch_link = '/stream/movie/%s.json'
		self.tvSearch_link = '/stream/series/%s:%s:%s.json'
		self.min_seeders = 0
# Currently supports YTS(+), EZTV(+), 1337x(+), TorrentGalaxy(+).
	...
	def sources(self, data, hostDict):
		...
		try:
			...
			results = session.get(url, timeout=5) # client.request(url, timeout=5)
			files = results.json()['streams'] # jsloads(results)['streams']
			_INFO = re.compile(r'👤.*')
			undesirables = source_utils.get_undesirables()
			check_foreign_audio = source_utils.check_foreign_audio()
		except:
			source_utils.scraper_error('ARAEC')
			return sources
		...
		for file in files:
			try:
				...
				append({'provider': 'araec', 'source': 'torrent', 'seeders': seeders, 'hash': hash, 'name': name, 'name_info': name_info, 'quality': quality,
	...
	def sources_packs(self, data, hostDict, search_series=False, total_seasons=None, bypass_filter=False):
		...
		for file in files:
			try:
				...
				item = {'provider': 'araec', 'source': 'torrent', 'seeders': seeders, 'hash': hash, 'name': name, 'name_info': name_info, 'quality': quality,
