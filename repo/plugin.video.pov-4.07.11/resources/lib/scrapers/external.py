...
#		self.sources, self.final_sources, self.processed_internal_scrapers = [], [], []
	...
	def get_sources(self):
		def _scraperDialog():
			...
			len_source_dict = len(self.source_dict)
			hide_busy_dialog()
			...
			while not self.progress_dialog.iscanceled() or monitor.abortRequested():
				try:
					...
					progress = int((len_source_dict-len_alive_threads)/len_source_dict*100)
			...
			return
		def _background():
			sleep(len(self.source_dict)*500)
			end_time = time.time() + self.timeout
...
#						if 'package' in i and provider != 'torrentio':
						if 'package' in i and provider not in ('torrentio', 'araec', 'elfhosted', 'nyaaio'):
	...
	def process_torrents(self, torrent_sources):
		...
		try:
			...
			cached_hashes = DebridCheck().run(hash_list, self.background, self.debrid_torrents, self.meta, self.progress_dialog)
