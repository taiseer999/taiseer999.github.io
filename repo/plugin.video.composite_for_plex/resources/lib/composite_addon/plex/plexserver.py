...
# --- البارامترات الأساسية لجلب الجودة والشعارات لكل المكتبة ---
_BASE_ARGS = {
    'includeGuids': '1',
    'checkFiles': '1',
    'includeBandwidths': '1',
    'includeAdvanced': '1',
    'hdr': '1',          # 🌟 السر المسروق من PM4K لإجبار بليكس على إرسال الـ HDR
    'dovi': '1',         # 🌟 السر الثاني لإجبار بليكس على إرسال بيانات Dolby Vision
}
...
class PlexMediaServer:
    ...
    # ── دوال جلب المحتوى (جميعها تستخدم _BASE_ARGS) ──────────────
    ...
    def get_watchlist(self, start=0, size=0):
        # 🌟 الاعتماد على السيرفر المحلي لمطابقة القائمة السحابية مع الملفات المتوفرة!
        args = dict(_BASE_ARGS)
        args.update({
            'type': '99', # 99 تعني دمج الأفلام والمسلسلات في قائمة واحدة
            'includeAdvanced': '1',
            'includeMeta': '1'
        })
        if size > 0: 
        ...
        # نطلب المسار الصحيح من السيرفر المحلي مباشرة
    ...
    def get_ondeck(self, section=-1, start=0, size=0):
        args = dict(_BASE_ARGS)
        args['includeAdvanced'] = '1'
        if size > 0: args.update({'X-Plex-Container-Start': start, 'X-Plex-Container-Size': size})
        path = '/library/onDeck' if section < 0 else '/library/sections/%s/onDeck' % section
        return self.processed_xml(self._update_path(path, args))
    ...
    def processed_xml(self, url):
        # 🌟 تصحيح أي طلب قديم لقائمة المشاهدة (المسار الميت) وتوجيهه للمسار السليم
        ...
        # 🌟 تأمين الروابط وضمان includeGuids
    ...
    def get_metadata(self, media_id):
        args = dict(_BASE_ARGS)
        args['includeAdvanced'] = '1'
        return self.processed_xml(self._update_path('/library/metadata/%s' % media_id, args))

    def get_children(self, media_id):
        args = dict(_BASE_ARGS)
        args['includeAdvanced'] = '1'
        return self.processed_xml(self._update_path('/library/metadata/%s/children' % media_id, args))

    def get_all_leaves(self, media_id):
        args = dict(_BASE_ARGS)
        return self.processed_xml(self._update_path('/library/metadata/%s/allLeaves' % media_id, args))
