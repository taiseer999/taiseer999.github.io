...
def random_valid_type_check():
	...
	'build_in_progress_episode': 'single_episode', 'build_recently_watched_episode': 'single_episode', 'build_next_episode': 'single_episode', 'build_random_next_episode': 'single_episode',
...
def random_episodes_check():
	...
	'build_next_episode': 'episode.next', 'build_random_next_episode': 'episode.next_random', 'build_my_calendar': 'episode.trakt'}
...
def addon_info(info):
	return xbmcaddon.Addon('plugin.video.fenskeleton').getAddonInfo(info)

def addon_version():
	return get_property('fenskeleton.addon_version') or addon_info('version')

def addon_path():
	return get_property('fenskeleton.addon_path') or addon_info('path')

def addon_profile():
	return get_property('fenskeleton.addon_profile') or translate_path(addon_info('profile'))

def addon_icon():
	return get_property('fenskeleton.addon_icon') or translate_path(addon_info('icon'))

def addon_icon_mini():
	return get_property('fenskeleton.addon_icon_mini') or os.path.join(addon_info('path'), 'resources', 'media', 'addon_icons', 'minis',
														os.path.basename(translate_path(addon_info('icon'))))

def addon_fanart():
	return get_property('fenskeleton.addon_fanart') or translate_path(addon_info('fanart'))

def get_icon(image_name, image_folder='icons', image_type='png'):
	# FenSkeleton: prefer bundled local art. Remote GitHub icon URLs were causing 404s before a repo is published.
	local_icon = os.path.join(addon_path(), 'resources', 'media', image_folder, '%s.%s' % (image_name, image_type))
	if path_exists(local_icon): return local_icon
	return os.path.join(addon_path(), 'resources', 'media', image_folder, 'folder.%s' % image_type)

def get_addon_fanart():
	return get_property('fenskeleton.default_addon_fanart') or addon_fanart()

def build_url(url_params):
	return 'plugin://plugin.video.fenskeleton/?%s' % urlencode(url_params)
...
def set_view_mode(view_type, content='files', is_external=None):
	if not get_property('fenskeleton.use_viewtypes') == 'true': return
	if is_external == None: is_external = external()
	if is_external: return
	view_id = get_property('fenskeleton.%s' % view_type) or None
...
def addon(addon_id='plugin.video.fenskeleton'):
...
def run_addon(addon='plugin.video.fenskeleton', block=False):
	return execute_builtin('RunAddon(%s)' % addon, block)

def external():
	return 'fenskeleton' not in get_infolabel('Container.PluginName')
...
def refresh_widgets():
	...
	kodi_refresh()
	if get_setting('fenskeleton.widget_refresh_notification', 'true') == 'true': notification('Widgets Refreshed', 2500)
...
def disable_enable_addon(addon_name='plugin.video.fenskeleton'):
	import json
	try:
...
def update_kodi_addons_db(addon_name='plugin.video.fenskeleton'):
	import time
...
def notification(line1, time=5000, icon=None):
	kodi_dialog().notification('FenSkeleton', line1, icon or addon_icon(), time)
...
def player_check(mode, params):
	...
	if mode == 'playback.%s' % playback_key():
		...
		from modules.player import FenSkeletonPlayer
		FenSkeletonPlayer().run(params.get('url', None), params.get('obj', None))
...
def external_playback_check(params):
	return True  # FenSkeleton: always allow external calls
...
def volume_checker():
	# 0% == -60db, 100% == 0db
	try:
		if get_property('fenskeleton.playback.volumecheck_enabled') == 'false' or get_visibility('Player.Muted'): return
		from modules.utils import string_alphanum_to_num
		max_volume = min(int(get_property('fenskeleton.playback.volumecheck_percent') or '50'), 100)
...
def get_all_icons():
	...
	username, location = get_property('fenskeleton.update.username'), get_property('fenskeleton.update.location')
...
def upload_logfile(params):
	...
	try:
		...
		if 'key' in response:
			...
			copy2clip(url)
			# Save the last uploaded log URL somewhere easy to retrieve from Android/TV boxes.
			try:
				out_dir = translate_path('special://profile/addon_data/plugin.video.fenskeleton/')
				make_directories(out_dir)
				out_file = out_dir + 'last_log_upload.txt'
				f = open_file(out_file, 'w')
				f.write('FenSkeleton %s upload:\n%s\n' % (log_name, url))
				f.close()
			except Exception as e:
				logger('FenSkeleton log url save failed', str(e))
			qr_code = make_qrcode(url) or ''
			ok_dialog(heading='Kodi Log Uploader', text='Log uploaded successfully.[CR][CR]URL copied to clipboard:[CR][B]%s[/B][CR][CR]Also saved to:[CR]special://profile/addon_data/plugin.video.fenskeleton/last_log_upload.txt' % url)
			progressDialog = progress_dialog(heading='Kodi Log Uploader', icon=qr_code)
			count, success = 12, None
