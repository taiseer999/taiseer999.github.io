...
def delete_all_ratings():
    import xbmcgui, xbmcvfs, sqlite3 as database
    dialog = xbmcgui.Dialog()
    if dialog.yesno("Fen-Mage", "Clear all cached ratings? This cannot be undone."):
        db_path = xbmcvfs.translatePath(
            "special://profile/addon_data/script.fentastic.helper/ratings_cache.db"
        )
        dbcon = database.connect(db_path, timeout=60)
        dbcon.execute("DELETE FROM ratings")
        dbcon.commit()
        dbcon.close()
