...
def widget_monitor(list_id):
    ...
    while not monitor.abortRequested():
        ...
        if window_id not in [10000, 1121]:
            break
        else:
        ...
        if list_id != str(window.getFocusId()):
            break
        last_path = window.getProperty("fenmage.%s.path" % list_id)
        ...
        while not monitor.abortRequested() and countdown >= 0 and switch_widget:
            ...
            if xbmc.getCondVisibility("System.HasActiveModalDialog"):
                switch_widget = False
            if xbmcgui.getCurrentWindowId() not in [10000, 1121]:
                switch_widget = False
        ...
        if switch_widget:
            ...
            stack_label_control.setLabel(
                window.getProperty("fenmage.%s.label" % list_id)
            )
            monitor.waitForAbort(0.25)
    try:
        del monitor
