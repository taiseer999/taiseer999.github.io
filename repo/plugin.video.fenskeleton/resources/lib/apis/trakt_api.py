...
def trakt_get_device_code():
	...
	try: logger('FenSkeleton Trakt device code result', 'ok' if result and result.get('user_code') else str(result))
	except Exception: pass
	return result

def trakt_get_device_token(device_codes):
	if not device_codes:
		try: logger('FenSkeleton Trakt auth stopped', 'no device code returned')
		except Exception: pass
		return None
	API_ENDPOINT = 'https://api.trakt.tv/%s'
	CLIENT_ID = settings.trakt_client()
	...
	try:
		...
		content = 'Enter [B]%s[/B] at [B]%s[/B][CR]OR....[CR]Scan the [B]QR Code[/B]%s[CR][CR]Checking every %s seconds.' % (user_code, device_codes['verification_url'], p_dialog_insert, sleep_interval)
		...
		while not progressDialog.iscanceled():
			...
			if time_passed >= expires_in:
				try: logger('FenSkeleton Trakt poll timeout', 'no token after %s seconds' % int(time_passed))
				except Exception: pass
				break
...
def trakt_refresh_token():
	try:
		...
		CLIENT_SECRET = settings.trakt_secret()
		if CLIENT_SECRET in (None, 'empty_setting', ''): return no_secret_key()
		kodi_utils.set_property('fenskeleton.trakt_refreshing_token', 'true')
		data = {        
			'client_id': CLIENT_ID, 'client_secret': CLIENT_SECRET, 'redirect_uri': 'urn:ietf:wg:oauth:2.0:oob',
			'grant_type': 'refresh_token', 'refresh_token': get_setting('fenskeleton.trakt.refresh')}
	...
	kodi_utils.clear_property('fenskeleton.trakt_refreshing_token')

def trakt_authenticate(dummy=''):
	try: logger('FenSkeleton Trakt authenticate', 'started')
	except Exception: pass
	code = trakt_get_device_code()
	token = trakt_get_device_token(code)
	if token:
		try: logger('FenSkeleton Trakt token', 'received')
		except Exception: pass
		...
		# Do not leave Trakt looking inactive if the final /users/me lookup fails.
		# The token is the important part for Next Up and sync.
		set_setting('trakt.user', 'Authorized')
		kodi_utils.sleep(1000)
		try:
			user = call_trakt('users/me')
			if user and user.get('username'): set_setting('trakt.user', str(user['username']))
		except Exception as e:
...
def trakt_revoke_authentication(dummy=''):
	...
	CLIENT_SECRET = settings.trakt_secret()
	if CLIENT_SECRET in (None, 'empty_setting', ''): return no_secret_key()
	data = {'token': get_setting('fenskeleton.trakt.token'), 'client_id': CLIENT_ID, 'client_secret': CLIENT_SECRET}
...
def trakt_calendar_days(recently_aired, current_date):
	if recently_aired: start, finish = (current_date - timedelta(days=14)).strftime('%Y-%m-%d'), '14'
		...
		previous_days = int(get_setting('fenskeleton.trakt.calendar_previous_days', '0'))
		future_days = int(get_setting('fenskeleton.trakt.calendar_future_days', '7'))
...
def trakt_sync_activities(force_update=False):
	...
	def refresh_token_check():
		...
		sync_interval = int(get_setting('fenskeleton.trakt.sync_interval', '60')) * 60
		try: expires_at = float(get_setting('fenskeleton.trakt.expires'))
		except: expires_at = 0.0
