...
class SettingsCache:
	def get(self, setting_id):
		try:
			...
			setting_id = setting_id.replace('fenskeleton.', '')
	...
	def set_memory_cache(self, setting_id, setting_value):
		kodi_utils.set_property('fenskeleton.%s' % setting_id, setting_value)

	def delete_memory_cache(self, setting_id):
		clear_property('fenskeleton.%s' % setting_id)
...
def _set_coco_defaults():
	...
	try:
		...
		if settings_cache.get('autoplay.episode') in (None, 'empty_setting', ''):
			settings_cache.set('autoplay.episode', 'true')
	except: pass
...
def set_boolean(params):
	...
	set_setting(setting, boolean_dict[get_setting('fenskeleton.%s' % setting)])

def set_string(params):
	current_value = get_setting('fenskeleton.%s' % params['setting_id'])
...
def set_path(params):
	...
	new_value = kodi_utils.kodi_dialog().browse(browse_mode, '', '', defaultt=get_setting('fenskeleton.%s' % setting_id))
	set_setting(setting_id, new_value)
...
def set_source_folder_path(params):
	setting_id = params['setting_id']
	current_setting = get_setting('fenskeleton.%s' % setting_id)
...
def default_setting_values(setting_id):
	if 'fenskeleton.' in setting_id: setting_id = setting_id.replace('fenskeleton.', '')
	d_settings = default_settings()
...
#===============================================================================#
#==================== General
{'setting_id': 'auto_start_Fenlight-Skeleton', 'setting_type': 'boolean', 'setting_default': 'false'},
...
{'setting_id': 'update.username', 'setting_type': 'string', 'setting_default': 'fenskeleton'},
{'setting_id': 'update.location', 'setting_type': 'string', 'setting_default': 'fenskeleton.github.io'},
#==================== Watched Indicators
{'setting_id': 'watched_indicators', 'setting_type': 'action', 'setting_default': '0', 'settings_options': {'0': 'FenSkeleton', '1': 'Trakt'}},
#======+============= Trakt Cache
...
#==================== Downloads
{'setting_id': 'movie_download_directory', 'setting_type': 'path', 'setting_default': 'special://profile/addon_data/plugin.video.fenskeleton/Movies Downloads/', 'browse_mode': '0'},
{'setting_id': 'tvshow_download_directory', 'setting_type': 'path', 'setting_default': 'special://profile/addon_data/plugin.video.fenskeleton/TV Show Downloads/', 'browse_mode': '0'},
{'setting_id': 'premium_download_directory', 'setting_type': 'path', 'setting_default': 'special://profile/addon_data/plugin.video.fenskeleton/Premium Downloads/', 'browse_mode': '0'},
{'setting_id': 'image_download_directory', 'setting_type': 'path', 'setting_default': 'special://profile/addon_data/plugin.video.fenskeleton/Image Downloads/', 'browse_mode': '0'},
