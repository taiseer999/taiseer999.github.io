	...
	def _process():
		for item in episodes_data:
			try:
				...
				set_properties({
					'episode_type': episode_type, 'fenskeleton.extras_params': extras_params, 'fenskeleton.options_params': options_params,
...
def build_single_episode(list_type, params={}):
	def _get_category_name():
		try:
			cat_name = {'episode.progress': 'In Progress Episodes',
						...
						'episode.next_trakt': 'Next Episodes', 'episode.next_fenskeleton': 'Next Episodes',
						'episode.next_random_trakt': 'Random Next Episodes', 'episode.next_random_fenskeleton': 'Random Next Episodes',
	...
	play_mode = 'playback.%s' % playback_key
	watched_db = ws.get_database(watched_indicators)
	randomize_next = list_type == 'episode.next_random'
	if list_type in ('episode.next', 'episode.next_random'):
	...
	if list_type_starts_with('next_random'):
		# FenSkeleton: Random Next Up must be genuinely random, not just a renamed Next Up widget.
		# Shuffle after metadata/build filtering so unaired/hidden/watched rules still apply.
		try:
