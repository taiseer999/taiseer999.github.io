...
class Navigator:
	def __init__(self, params):
		...
		self.category_name = self.params_get('name', 'FenSkeleton')
	...
	def tools(self):
		...
		self.add({'mode': 'open_skin_menu_settings', 'isFolder': 'false'}, 'Skin Menu & Widget Setup', 'settings2')
		if get_setting('fenskeleton.external_scraper.module') not in ('empty_setting', ''):
	...
	def changelog_utils(self):
		...
		fenskeleton_clogpath = k.translate_path('special://home/addons/plugin.video.fenskeleton/resources/text/changelog.txt')
		self.add({'mode': 'show_text', 'heading': 'Changelog', 'file': fenskeleton_clogpath, 'font_size': 'large', 'isFolder': 'false'}, 'Changelog', 'lists')
	...
	def build_shortcut_folder_contents(self):
		...
		if contents:
			for count, item in enumerate(contents):
				...
				icon = iconImage
				if iconImage:
					if iconImage.startswith('http') or 'plugin.video.fenskeleton' in iconImage: original_image = True
	...
	def exit_media_menu(self):
		params = k.get_property('fenskeleton.exit_params')
		if params: return k.container_refresh_input(params)

	def tips(self):
		tips_location = 'special://home/addons/plugin.video.fenskeleton/resources/text/tips'
