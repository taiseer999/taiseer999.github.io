...
def window_theme_choice(params):
	if params['type'] == 'theme':
		...
		window_theme_opacity = get_setting('fenskeleton.window_theme_opacity', 'CC')
		...
		window_theme = get_setting('fenskeleton.window_theme', 'FF1F2020')[2:]
		window_theme_contrast = get_setting('fenskeleton.window_theme_contrast', 'FF4a4347')
...
def language_invoker_choice(params):
	...
	addon_xml = kodi_utils.translate_path('special://home/addons/plugin.video.fenskeleton/addon.xml')
...
def addon_icon_choice(params):
	...
	if set_icon: new_name = set_icon
		...
		try:
			...
			if not all_icons: kodi_utils.ok_dialog(heading='FenSkeleton Icon Images', text='Error Fetching Icon Images')
		...
		except: return kodi_utils.ok_dialog(heading='FenSkeleton Icon Images', text='Error Fetching Addon Icon Images') 
...
def personallists_manager_choice(params):
	...
	list_type = params['list_type']
	all_lists = get_all_personal_lists(get_setting('fenskeleton.personal_list.list_sort', '0'))
	choices = []
...
def external_scraper_choice(params):
	...
	if success:
		try:
			set_setting('external_scraper.name', module_id)          # full addon ID → used as addon folder path
			set_setting('external_scraper.module', main_folder_name) # bare module name → used for Python import
...
def set_quality_choice(params):
	...
	fl = ['4K', '1080p', '720p', 'SD']
	q_setting = get_setting('fenskeleton.%s' % quality_setting).split(', ')
	...
	if not orig_button_dict:
		for _type in ('movie', 'tvshow'):
			...
			for item in range(10, 18):
				...
				try:
					button_action = get_setting('fenskeleton.%s' % setting_id)
					button_label = extras_button_label_values[_type][button_action]
				except:
					set_setting(setting_id.replace('fenskeleton.', ''), default_setting_values(setting_id)['setting_default'])
					button_action = get_setting('fenskeleton.%s' % setting_id)
					button_label = extras_button_label_values[_type][button_action]
...
def set_language_filter_choice(params):
	...
	set_filter = get_setting('fenskeleton.%s' % filter_setting_id).split(', ')
...
def enable_scrapers_choice(params={}):
	icon = params.get('icon', None) or kodi_utils.get_icon('fenskeleton')
...
def scraper_color_choice(params):
	...
	current_setting, original_highlight = get_setting('fenskeleton.%s' % setting), default_setting_values(setting)['setting_default']
...
def options_menu_choice(params, meta=None):
	...
	single_ep_list = ('episode.progress', 'episode.recently_watched', 'episode.next_trakt', 'episode.next_fenskeleton', 'episode.trakt_recently_aired', 'episode.trakt_calendar')
