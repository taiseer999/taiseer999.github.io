...
class PremAccntNotification:
	def run(self):
		...
		for name, expires, module, cls in (
			('alldebrid.username', 'alldebrid.expires', 'alldebrid', 'AllDebrid'),
			('premiumize.username', 'premiumize.expires', 'premiumize', 'Premiumize'),
			('realdebrid.username', 'realdebrid.expires', 'realdebrid', 'RealDebrid'),
			('easydebrid.username', 'easydebrid.expires', 'easydebrid', 'EasyDebrid'),
			('torbox.username', 'torbox.expires', 'torbox', 'TorBox')
		):
			try:
				if not control.setting(name): continue
				if limit := int(control.setting(expires, '7')):
					module = 'resources.lib.debrid.%s' % module
					...
					if not days_remaining is None and days_remaining <= limit:
						control.notification(message='%s expires in %s days' % (cls.name, days_remaining))
			except:
				log_utils.error()
