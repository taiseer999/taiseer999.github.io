...
import inspect
from resources.lib.fenom.control import transPath, setting as getSetting, lang, joinPath, existsPath
...
def clear_logFile():
	cleared = False
	try:
		from resources.lib.fenom.control import yesnoDialog
...
def view_LogFile(name):
	try:
		from resources.lib.fenom.textviewer import TextViewerXML
		from resources.lib.fenom.control import addonPath
		log_file = joinPath(LOGPATH, '%s.log' % name.lower())
		if not existsPath(log_file):
			from resources.lib.fenom.control import notification
...
def upload_LogFile():
	...
	try:
		import requests
		from resources.lib.fenom.control import addonVersion, selectDialog
		f = open(log_file, 'r', encoding='utf-8', errors='ignore')
		text = f.read()
		f.close()
		...
		if 'key' in response.json():
			...
			if 'Copy url To Clipboard' in list[select][0]:
				from resources.lib.fenom.source_utils import copy2clip
