			...
			if action_id in self.selection_actions:
				...
				if 'UNCACHED' in source:
					...
					self.execute_code('RunPlugin(plugin://plugin.video.dradis/?action=cacheTorrent&caller=%s&type=%s&title=%s&query=%s&items=%s&url=%s&source=%s&meta=%s)' %
											(debrid, link_type, sysname, new_sysname, quote_plus(jsdumps(self.uncached)), quote_plus(chosen_source.getProperty('dradis.url')), quote_plus(source_dict), quote_plus(jsdumps(self.meta))))
					self.selected = (None, '')
				else:
					self.selected = (None, '')
				return self.close()
