...
#		if not self.API_key: self.API_key = ''
...
# revenue
	...
	def get_art(self, tmdb):
		...
		try:
			logo = self.parse_art(art3['logos'])
			logo = '%s%s' % (self.poster_path, logo) if logo else ''
		except: logo = ''
		try:
		...
		extended_art = {'extended': True, 'poster3': poster3, 'fanart3': fanart3, 'tmdblogo': logo}
		return extended_art
...
class TVshows(TMDb):
	...
	def get_seasonEpisodes_meta(self, tmdb, season): # builds episodes meta from "/season/?" request
		...
		try:
			...
			for episode in result['episodes']:
				...
				episode_meta['episode_type'] = episode['episode_type']
				if   episode_meta['episode_type'] == 'mid_season': ep_details[episode['episode_number'] + 1] = 'mid_season_premiere'
				if   episode_meta['episode_type'] in ep_details: episode_meta['episode_type'] = ep_details[episode_meta['episode_type']]
				elif episode['episode_number'] in ep_details: episode_meta['episode_type'] = ep_details[episode['episode_number']]
	...
	def get_episode_groups(self, tmdb):
		eps_map = {1: 'Original air date', 2: 'Absolute', 3: 'DVD', 4: 'Digital', 5: 'Story arc', 6: 'Production', 7: 'TV'}
		url = '%stv/%s/episode_groups?api_key=%s' % (base_link, tmdb, self.API_key)
		result = cache.get(self.get_request, 96, url)
		for i in result['results']: i['type'] = eps_map[i['type']]
		result = result['results']
		return result

	def get_episode_group_details(self, group):
		try:
			url = '%stv/episode_group/%s?api_key=%s' % (base_link, group, self.API_key)
			result = cache.get(self.get_request, 96, url)
