...
from resources.lib.fenom import sources as fs_sources
		...
		while True:
			try:
				...
				try:
					...
					else: self.progressDialog.update(max(1, percent), line1 + '  ' + string3 % str(len(info)))
				...
				while True:
					try:
						...
						try:
							...
							if not info: break
							if self.progressDialog != control.progressDialogBG: self.progressDialog.update(-1, f"{line1}[CR]{line2}[CR]{line3}")
							else: self.progressDialog.update(-1, line1 + '  ' + line3)
						except:
							log_utils.error()
							break
						control.sleep(25)
...
#			control.notification(title=item_title, message='Removed %s duplicate sources from list' % (len(self.sources) - len(filter)))
	...
	def sourcesResolve(self, item):
		...
		if 'magnet:' in url:
			...
			try:
				direct = item['direct']
				if direct:
					direct_sources = ('ad_cloud', 'oc_cloud', 'pm_cloud', 'rd_cloud')
	...
	def getConstants(self):
		...
		if getSetting('filepursuit.api'): self.prem_providers += [('filepursuit', int(getSetting('filepursuit.priority')))]
		if getSetting('gdrive.url'): self.prem_providers += [('gdrive', int(getSetting('gdrive.priority')))]
		if getSetting('ororo.username'): self.prem_providers += [('ororo', int(getSetting('ororo.priority')))]
...
#				if i['provider'] == 'torrentio': continue # torrentio return file size based on episode query already so bypass re-calc
				if i['provider'] in ('torrentio', 'araec', 'elfhosted', 'nyaaio'): continue
				if 'package' in i:
					dsize = i.get('size')
	...
	def rd_cache_chk_list(self, torrent_List, hashList):
		def _process(hashes):
			try: cached.update(RealDebrid().check_cache(hashes))
			except: pass
		if len(torrent_List) == 0: return
		try:
			...
			cached = {}
			threads = []
			for i in (hashList[i:i + 10] for i in range(0, len(hashList), 10)):
				obj = Thread(target=_process, args=(i,))
				threads.append(obj)
				obj.start()
			[i.join() for i in threads]
