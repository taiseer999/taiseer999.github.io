...
def setting(id, fallback=None):
	try: settings_dict = jsloads(homeWindow.getProperty('dradis_settings'))
...
def make_settings_dict(): # service runs upon a setting change
	try:
		...
		homeWindow.setProperty('dradis_settings', jsdumps(settings_dict))
		refresh_playAction()
		refresh_libPath()
...
def openSettings(query=None, id=addonInfo('id')):
	try:
		...
		if not query: return
		c, f = query.split('.')
		v1, v2 = (200, 180) if KODI_VERSION > 20 else (100, 80)
		execute('SetFocus(%i)' % (int(c) - v1))
#		execute('SetFocus(%i)' % (int(f) - v2))
	except:
...
def getDradisVersion():
	return xbmcaddon.Addon('plugin.video.dradis').getAddonInfo('version')
...
def artPath():
	theme = appearance()
	return joinPath(xbmcaddon.Addon('plugin.video.dradis').getAddonInfo('path'), 'resources', 'media', theme)

def genreIconPath():
	return joinPath(xbmcaddon.Addon('plugin.video.dradis').getAddonInfo('path'), 'resources', 'media', 'genre_media', 'icons')

def genrePosterPath():
	return joinPath(xbmcaddon.Addon('plugin.video.dradis').getAddonInfo('path'), 'resources', 'media', 'genre_media', 'posters')
...
def getColor(n):
	colorChart = ('cyan', 'deepskyblue', 'blue', 'skyblue', 'magenta', 'deeppink', 'red', 'gold', 'yellow',
...
def refresh_playAction(): # for dradis global CM play actions
	autoPlay = 'true' if setting('play.mode') == '1' else ''
	homeWindow.setProperty('dradis.autoPlay.enabled', autoPlay)

def refresh_libPath(): # for dradis global CM library actions
	homeWindow.setProperty('dradis.movieLib.path', transPath(setting('library.movie')))
	homeWindow.setProperty('dradis.tvLib.path', transPath(setting('library.tv')))

def refresh_debugReversed(): # called from service "onSettingsChanged" to clear dradis.log if setting to reverse has been changed
	if homeWindow.getProperty('dradis.debug.reversed') != setting('debug.reversed'):
		homeWindow.setProperty('dradis.debug.reversed', setting('debug.reversed'))
		execute('RunPlugin(plugin://plugin.video.dradis/?action=tools_clearLogFile)')
