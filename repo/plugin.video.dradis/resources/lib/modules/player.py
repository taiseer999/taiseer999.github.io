	...
	def get(self, name, imdb, season, episode):
		...
		try:
			...
			if not (season is None or episode is None):
				url = '/episode-%s/imdbid-%s/season-%s' % (episode, imdb.lstrip('t'), season)
				fmt = ['hdtv']
			else:
				url = '/imdbid-%s' % imdb.lstrip('t')
			...
			filter = []
			for lang in langs:
				link = base_url + url + '/sublanguageid-%s' % lang
				result = requests.get(link, headers=headers, timeout=timeout)
				result = result.json()
			...
			content = filter[0]['SubDownloadLink']
			result = requests.get(content, headers=headers, stream=True, timeout=timeout)
			content = gzip.GzipFile(fileobj=BytesIO(result.content)).read()
