...
##################
	...
	def keepAlive(self):
		...
		if playerWindow.getProperty('dradis.playlistStart_position'): pass
		else:
			if control.playlist.size() > 1: playerWindow.setProperty('dradis.playlistStart_position', str(control.playlist.getposition()))
						...
						if self.enable_playnext and not self.play_next_triggered:
							if int(control.playlist.size()) > 1:
								if self.preScrape_triggered == False:
									xbmc.executebuiltin('RunPlugin(plugin://plugin.video.dradis/?action=play_preScrapeNext)')
								...
								if remaining_time < (self.playnext_time + 1) and remaining_time != 0:
									xbmc.executebuiltin('RunPlugin(plugin://plugin.video.dradis/?action=play_nextWindowXML)')
...
### Kodi player callback methods ###
	def onAVStarted(self): # Kodi docs suggests "Use onAVStarted() instead of onPlayBackStarted() as of v18"
		...
		xbmc.log('[ plugin.video.dradis ] onAVStarted callback', LOGINFO)
		log_utils.log('[ plugin.video.dradis ] onAVStarted callback', level=log_utils.LOGDEBUG)

	def onPlayBackSeek(self, time, seekOffset):
		seekOffset /= 1000

	def onPlayBackSeekChapter(self, chapter):
		log_utils.log('[ plugin.video.dradis ] onPlayBackSeekChapter callback', level=log_utils.LOGDEBUG)

	def onQueueNextItem(self):
		log_utils.log('[ plugin.video.dradis ] onQueueNextItem callback', level=log_utils.LOGDEBUG)
	...
	def onPlayBackStopped(self):
		try:
			...
			playerWindow.clearProperty('dradis.playlistStart_position')
			homeWindow.clearProperty('dradis.source_progress_is_alive')
			clear_local_bookmarks() # clear all dradis bookmarks from kodi database
			...
			if not self.onPlayBackStopped_ran or (self.playbackStopped_triggered and not self.onPlayBackStopped_ran): # Kodi callback unreliable and often not issued
				...
				if getSetting('crefresh') == 'true' and seekable:
					log_utils.log('[ plugin.video.dradis ] container.refresh issued', level=log_utils.LOGDEBUG)
				...
				xbmc.log('[ plugin.video.dradis ] onPlayBackStopped callback', LOGINFO)
				log_utils.log('[ plugin.video.dradis ] onPlayBackStopped callback', level=log_utils.LOGDEBUG)
		except: log_utils.error()
	...
	def onPlayBackEnded(self):
		...
		if control.playlist.getposition() == control.playlist.size() or control.playlist.size() == 1:
			control.playlist.clear()
		xbmc.log('[ plugin.video.dradis ] onPlayBackEnded callback', LOGINFO)
		log_utils.log('[ plugin.video.dradis ] onPlayBackEnded callback', level=log_utils.LOGDEBUG)
	...
	def onPlayBackError(self):
		...
		Bookmarks().reset(self.current_time, self.media_length, self.name, self.year)
		log_utils.error()
		xbmc.log('[ plugin.video.dradis ] onPlayBackError callback', LOGINFO)
		log_utils.log('[ plugin.video.dradis ] onPlayBackError callback', level=log_utils.LOGDEBUG)
		sysexit(1)
##############################
...
class PlayNext(xbmc.Player):
	...
	def isStill_watching(self):
		...
		playlistStart_position = int(playerWindow.getProperty('dradis.playlistStart_position'))
...
class Subtitles:
	def get(self, name, imdb, season, episode):
		...
		try:
			...
			langs = [xbmc.convertLanguage(i, xbmc.ISO_639_1) for i in langs]
			base_url = 'https://sub.wyzie.ru/search'
			timeout = 5.05

			params = {'id': imdb, 'format': 'srt'}
			...
			except: transname = name.translate(name.maketrans('', '', r'\/:*?"<>|')).strip('.')  # maketrans() is in string module for py2
			source_site = filter[0]['source']
			filename = f"{transname} {imdb} {lang} {filter[0]['format']}".replace(' ', '.')
			log_utils.log('downloaded subtitle=%s' % filename, level=log_utils.LOGDEBUG)

			content = filter[0]['url']
			result = requests.get(content, stream=True, timeout=timeout)
			...
			if getSetting('subtitles.utf') == 'true':
				...
				if codepage and not filter[0].get('language', '').lower() == 'utf-8':
					try:
			...
			if getSetting('subtitles.notification') == 'true':
				if Player().isPlayback():
					control.sleep(500)
					control.notification(title=filename, message=getLS(32191) % (lang.upper(), source_site.upper()))
		except: log_utils.error()
