...
class Subtitles:
	def get(self, name, imdb, season, episode):
		...
		try:
			...
			session.headers = {'User-Agent': 'TemporaryUserAgent'}
			base_url = 'https://rest.opensubtitles.org/search'
			timeout = 3.05

			url = '/imdbid-%s/query-%s' % (imdb, requests.utils.quote(name))
			...
			for lang in langs:
				...
				result = session.get(link, timeout=timeout)
				result = result.json()
			...
			result = session.get(content, stream=True, timeout=timeout)
