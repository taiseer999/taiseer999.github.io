...
tmdb_api_key = getSetting('tmdb.api.key')
...
def getMovieOverlay(indicators, imdb):
	...
	try:
		if traktIndicators:
			...
			playcount = indicators.get_watched('movie', imdb, None)
...
def getEpisodeOverlay(indicators, imdb, tvdb, season, episode):
	...
	try:
		if traktIndicators:
			...
			playcount = indicators.get_watched_episode('episode', imdb, None, season=season, episode=episode)
			return str(playcount)
	except:
		from resources.lib.modules import log_utils
		log_utils.error()
