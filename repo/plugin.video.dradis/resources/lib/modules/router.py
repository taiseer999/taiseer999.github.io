...
def router(params):
	...
	if action is None:
		...
		isUpdate = control.homeWindow.getProperty('dradis.updated')
		if isUpdate == 'true':
			control.execute('RunPlugin(plugin://plugin.video.dradis/?action=tools_cleanSettings)')
			control.homeWindow.clearProperty('dradis.updated')
			from resources.lib.modules import changelog
			changelog.get('Dradis')
	...
	####################################################
		...
		if action == 'pm_ServiceNavigator':
			...
			from resources.lib.debrid import premiumize
			premiumize.Premiumize().delete_transfer(params.get('id'), name, silent=False)
		...
		elif action == 'rd_Transfers':
			from resources.lib.debrid import realdebrid
			realdebrid.RealDebrid().user_torrents_to_listItem()
		elif action == 'rd_CloudStorage':
			from resources.lib.debrid import realdebrid
			realdebrid.RealDebrid().user_cloud_to_listItem()
		elif action == 'rd_BrowseUserCloud':
			from resources.lib.debrid import realdebrid
			realdebrid.RealDebrid().browse_user_cloud(params.get('id'))
		...
		if action == 'tb_AccountInfo':
			...
			from resources.lib.debrid import torbox
			torbox.TorBox().user_cloud_to_listItem(params.get('mediatype'))
		...
		from resources.lib.modules import myservices
		myservices.authorize(query)
	elif action == 'speedTest':
		from resources.lib.fenom import speedtest
		speedtest.magneto()
		...
		if action == 'downloadNavigator':
			...
			if caller == 'torbox':
				...
				try:
					from resources.lib.modules import downloader
					from resources.lib.debrid import torbox
					if params.get('mediatype') == 'usenet':
						downloader.download(name, image, torbox.TorBox().unrestrict_usenet(url.replace(' ', '%20')))
					else:
						downloader.download(name, image, torbox.TorBox().unrestrict_link(url.replace(' ', '%20')))
				except:
					import traceback
					traceback.print_exc()
	...
	####################################################
		...
		if action == 'tools_ShowNews':
			...
			from resources.lib.menus import navigator
			navigator.Navigator().views(params.get('content'))
		...
		elif action == 'tools_contextDradisSettings':
			control.openSettings('0.0', 'context.dradis')
		...
		elif action == 'tools_mdblistManager':
			from resources.lib.indexers import mdblist
			mdblist.manager(name, imdb, params.get('media_type'))
			...
			elif cleared: control.notification(message='Dradis Log File Successfully Cleared')
			else: control.notification(message='Error clearing Dradis Log File, see kodi.log for more info')
	...
	####################################################
		...
		if action == 'play_Item':
			...
			for i in items:
				...
				url = 'plugin://plugin.video.dradis/?action=play_Item&title=%s&year=%s&imdb=%s&tmdb=%s&tvdb=%s&season=%s&episode=%s&tvshowtitle=%s&premiered=%s&meta=%s&select=1' % (
			...
			if rtype == 'movie':
				...
				r = 'plugin://plugin.video.dradis/?action=play_Item'
			elif rtype == 'episode':
				from resources.lib.menus import episodes
				...
				r = 'plugin://plugin.video.dradis/?action=play_Random&rtype=episode'
			elif rtype == 'show':
				from resources.lib.menus import tvshows
				rlist = tvshows.TVshows().get(url, create_directory=False)
				r = 'plugin://plugin.video.dradis/?action=play_Random&rtype=season'
	...
	####################################################
		...
		if url.startswith('magnet'): success = debrid_function().add_uncached_torrent(url, pack=pack)
		else: success = debrid_function().add_uncached_nzb(url, query, pack=pack)
		if success:
			from resources.lib.modules import sources
		...
		if tvshowtitle: items += ['Rescrape Item from [COLOR %s][B]Episode Group[/B][/COLOR] - Source Select' % highlight_color]
		...
		if select >= 0:
			...
			elif select == 7: sources.Sources(custom_query='group').play(title, year, imdb, tmdb, tvdb, season, episode, tvshowtitle, premiered, meta, select='0', rescrape='true')
	...
	####################################################
		...
		elif action == 'cache_clearKodiBookmark': # context.dradis action call only
