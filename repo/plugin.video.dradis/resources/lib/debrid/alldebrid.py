...
class AllDebrid:
	...
	def auth(self):
		...
		interval = 5
		verification_url = getLS(32513) % (control.getHighlightColor(), str(response['base_url']))
		user_code = getLS(32514) % (control.getHighlightColor(), str(response['pin']))
		try:
			qr_url = '&bgcolor=ffd700&data=%s' % quote_plus(response['user_url'])
			qr_icon = 'https://api.qrserver.com/v1/create-qr-code/?size=256x256&qzone=1%s' % qr_url
			control.notification(message=response['user_url'], icon=qr_icon, time=15000)
		except: pass
		line = '%s\n%s'
		...
		while not self.token:
			...
			if not time_passed % interval: self.auth_loop()
