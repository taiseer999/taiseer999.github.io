...
from resources.lib.fenom import source_utils as fs_utils
	...
	def sources(self, data, hostDict):
		...
		for folder in cloud_folders:
			...
			try:
				...
				id = folder.get('id', '')
				torrent_info = AllDebrid().transfer_info(id)
				files = AllDebrid.flatten_magnet_files(torrent_info['files'])
				files = [i for i in files if not i['n'].lower().endswith(invalid_extensions)]
			except:
				from resources.lib.modules import log_utils
				log_utils.error('AD_CLOUD: ')
				return sources
			...
			for file in files:
				try:
					...
					if name.endswith('m2ts'):
						...
						is_m2ts = True
						m2ts_files = [i for i in files if name == i.get('n')]
						largest = sorted(m2ts_files, key=lambda k: k['s'], reverse=True)[0]
						link = largest.get('l', '')
						size =  largest.get('s', '')
					else:
						if all(not bool(re.search(i, rt)) for i in query_list):
							if 'tvshowtitle' in data:
								...
								name = folder_name
								if file.get('s') < 52428800: continue
