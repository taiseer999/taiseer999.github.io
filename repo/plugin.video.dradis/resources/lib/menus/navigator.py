...
class Navigator:
	def __init__(self):
		...
		self.mdblCredentials = getSetting('mdblist.token') != ''
	...
	def root(self):
		...
		if getMenuEnabled('navi.anime'): self.addDirectoryItem('Discover Anime', 'anime_Navigator', 'boxsets.png', 'DefaultFolder.png')
		...
		if getMenuEnabled('navi.changelog'): self.addDirectoryItem(32014, 'tools_ShowChangelog&name=Dradis', 'userlists.png', 'DefaultAddonHelper.png', isFolder=False)
		self.endDirectory()
	...
	def movies(self, lite=False):
		...
		if getMenuEnabled('navi.movie.trakt.trendingrecent'):
			self.addDirectoryItem('Trending Recent (Trakt)' if self.indexLabels else 'Trending Recent', 'movies&url=trakttrendingrecent', 'trakt.png' if self.iconLogos else 'trending.png', 'trending.png')
...
#			self.addDirectoryItem(32421 if self.indexLabels else 32420, 'movies&url=theaters', 'imdb.png' if self.iconLogos else 'in-theaters.png', 'DefaultMovies.png')
		self.addDirectoryItem('Digital/DVD Releases (TMDb)' if self.indexLabels else 'Digital/DVD Releases', 'tmdbmovies&url=tmdb_digital', 'tmdb.png' if self.iconLogos else 'movies.png', 'DefaultMovies.png')
...
#			self.addDirectoryItem(32456 if self.indexLabels else 32455, 'movieGenres&url=genre', 'imdb.png' if self.iconLogos else 'genres.png', 'DefaultGenre.png')
		...
		if getMenuEnabled('navi.movie.trakt.genres'):
			self.addDirectoryItem('Genres (Trakt)' if self.indexLabels else 'Genres', 'movieGenres&url=traktpopular', 'trakt.png' if self.iconLogos else 'genres.png', 'DefaultGenre.png')
...
#			self.addDirectoryItem(32458 if self.indexLabels else 32457, 'movieYears&url=year', 'imdb.png' if self.iconLogos else 'years.png', 'DefaultYear.png')
		...
		if getMenuEnabled('navi.movie.trakt.years'):
			self.addDirectoryItem('Year (Trakt)' if self.indexLabels else 'Years', 'movieYears&url=traktpopular', 'trakt.png' if self.iconLogos else 'years.png', 'DefaultYear.png')
...
#			self.addDirectoryItem(32464 if self.indexLabels else 32463, 'movieCertificates&url=certification', 'imdb.png' if self.iconLogos else 'certificates.png', 'DefaultMovies.png')
		...
		if self.mdblCredentials: self.addDirectoryItem('MDBList Top Lists', 'movies&url=mdbltop', 'mdblist.png' if self.iconLogos else 'movies.png', 'DefaultMovies.png')
	...
	def mymovies(self, lite=False):
		...
		if self.mdblCredentials: self.addDirectoryItem('MDBList %s' % getLS(32033), 'movies&url=mdblwatchlist', 'mdblist.png', 'mdblist.png')
	...
	def tvshows(self, lite=False):
		...
		if getMenuEnabled('navi.tv.trakt.trendingrecent'):
			self.addDirectoryItem('Trending Recent (Trakt)' if self.indexLabels else 'Trending Recent', 'tvshows&url=trakttrendingrecent', 'trakt.png' if self.iconLogos else 'trending.png', 'DefaultTVShows.png')
...
#			self.addDirectoryItem(32456 if self.indexLabels else 32455, 'tvGenres&url=genre', 'imdb.png' if self.iconLogos else 'genres.png', 'DefaultGenre.png')
		...
		if getMenuEnabled('navi.tv.trakt.genres'):
			self.addDirectoryItem('Genres (Trakt)' if self.indexLabels else 'Genres', 'tvGenres&url=traktpopular', 'trakt.png' if self.iconLogos else 'genres.png', 'DefaultGenre.png')
...
#			self.addDirectoryItem(32458 if self.indexLabels else 32457, 'tvYears&url=year', 'imdb.png' if self.iconLogos else 'years.png', 'DefaultYear.png')
		...
		if getMenuEnabled('navi.tv.trakt.years'):
			self.addDirectoryItem('Year (Trakt)' if self.indexLabels else 'Year', 'tvYears&url=traktpopular', 'trakt.png' if self.iconLogos else 'years.png', 'DefaultYear.png')
...
#			self.addDirectoryItem(32476 if self.indexLabels else 32475, 'tvshows&url=premiere', 'imdb.png' if self.iconLogos else 'new-tvshows.png', 'DefaultRecentlyAddedEpisodes.png')
		...
		if self.mdblCredentials: self.addDirectoryItem('MDBList Top Lists', 'tvshows&url=mdbltop', 'mdblist.png' if self.iconLogos else 'tvshows.png', 'DefaultTVShows.png')
	...
	def mytvshows(self, lite=False):
		...
		if self.mdblCredentials: self.addDirectoryItem('MDBList %s' % getLS(32033), 'tvshows&url=mdblwatchlist', 'mdblist.png', 'mdblist.png')
...
#		self.addDirectoryItem(f"{getLS(32001)} (IMDB)" if self.indexLabels else 32001, 'anime_Movies&url=anime', 'movies.png', 'DefaultMovies.png')
#		self.addDirectoryItem(f"{getLS(32002)} (IMDB)" if self.indexLabels else 32002, 'anime_TVshows&url=anime', 'tvshows.png', 'DefaultTVShows.png')
		...
		self.addDirectoryItem('Series Trending (Trakt)' if self.indexLabels else 'Series Trending', 'tvshows&url=trakttrendinganime', 'tvshows.png', 'DefaultTVShows.png')
		self.addDirectoryItem('Series Most Watched (Trakt)' if self.indexLabels else 'Series Most Watched', 'tvshows&url=traktwatchedanime', 'tvshows.png', 'DefaultTVShows.png')
		self.addDirectoryItem('Series Popular (TMDb)' if self.indexLabels else 'Series Popular', 'tmdbTvshows&url=tmdb_popularanime', 'tvshows.png', 'DefaultTVShows.png')
		self.addDirectoryItem('Movies Trending (Trakt)' if self.indexLabels else 'Movies Trending', 'movies&url=trakttrendinganime', 'movies.png', 'DefaultMovies.png')
		self.addDirectoryItem('Movies Most Watched (Trakt)' if self.indexLabels else 'Movies Most Watched', 'movies&url=traktwatchedanime', 'movies.png', 'DefaultMovies.png')
		self.addDirectoryItem('Movies Popular (TMDb)' if self.indexLabels else 'Movies Popular', 'tmdbmovies&url=tmdb_popularanime', 'movies.png', 'DefaultMovies.png')
		self.endDirectory()
	...
	def traktSearchLists(self, media_type):
		...
		control.closeAll()
		if media_type == 'movies': control.execute('ActivateWindow(Videos,plugin://plugin.video.dradis/?action=movies_PublicLists&url=%s,return)' % (quote_plus(url)))
		else: control.execute('ActivateWindow(Videos,plugin://plugin.video.dradis/?action=tv_PublicLists&url=%s,return)' % (quote_plus(url)))
...
#		self.addDirectoryItem(32506, 'tools_contextDradisSettings', 'icon.png', 'DefaultAddonProgram.png', isFolder=False)
		#-- Providers - 5
		...
		self.addDirectoryItem(32049, 'tools_viewsNavigator', 'tools.png', 'DefaultAddonService.png', isFolder=False)
	...
	def loggingNavigator(self):
		self.addDirectoryItem(32524, 'tools_viewLogFile&name=Dradis', 'tools.png', 'DefaultAddonProgram.png', isFolder=False)
		self.addDirectoryItem(32525, 'tools_clearLogFile', 'tools.png', 'DefaultAddonProgram.png', isFolder=False)
#		self.addDirectoryItem(32526, 'tools_ShowChangelog&name=Dradis', 'tools.png', 'DefaultAddonProgram.png', isFolder=False)
		self.addDirectoryItem(32527, 'tools_uploadLogFile&name=Dradis', 'tools.png', 'DefaultAddonProgram.png', isFolder=False)
...
#		self.addDirectoryItem(32531, 'tools_ShowChangelog&name=FenomScrapers', 'tools.png', 'DefaultAddonProgram.png', isFolder=False)
	...
	def premium_services(self):
		...
		if getMenuEnabled('navi.offcloud'): self.addDirectoryItem('Offcloud', 'oc_ServiceNavigator', 'offcloud.png', 'offcloud.png')
		if getMenuEnabled('navi.torbox'): self.addDirectoryItem('TorBox', 'tb_ServiceNavigator', 'torbox.png', 'torbox.png')
	...
	def offcloud_service(self):
		if getSetting('offcloud.token'):
			...
			self.addDirectoryItem('[I]Please visit My Accounts for setup[/I]', 'tools_openSettings&query=6.0', 'offcloud.png', 'DefaultAddonService.png', isFolder=False)
		self.endDirectory()
	...
	def premiumize_service(self):
		if getSetting('premiumize.token'):
			...
			self.addDirectoryItem('[I]Please visit My Accounts for setup[/I]', 'tools_openSettings&query=6.0', 'premiumize.png', 'DefaultAddonService.png', isFolder=False)
		self.endDirectory()

	def realdebrid_service(self):
		if getSetting('realdebrid.token'):
			self.addDirectoryItem('Real-Debrid: Cloud Storage', 'rd_CloudStorage', 'realdebrid.png', 'DefaultAddonService.png')
			self.addDirectoryItem('Real-Debrid: Transfers', 'rd_Transfers', 'realdebrid.png', 'DefaultAddonService.png')
	...
	def torbox_service(self):
		if getSetting('torbox.token'):
			self.addDirectoryItem('TorBox: Torrent Cloud Storage', 'tb_CloudStorage', 'torbox.png', 'DefaultAddonService.png')
			self.addDirectoryItem('TorBox: Usenet Cloud Storage', 'tb_CloudStorage&mediatype=usenet', 'torbox.png', 'DefaultAddonService.png')
			self.addDirectoryItem('TorBox: Account Info', 'tb_AccountInfo', 'torbox.png', 'DefaultAddonService.png', isFolder=False)
		else:
			self.addDirectoryItem('[I]Please setup in Accounts[/I]', 'tools_openSettings&query=6.0', 'torbox.png', 'DefaultAddonService.png', isFolder=False)
		self.endDirectory()
	...
	def views(self, content=None):
		try:
			...
			if not content:
				...
				url = '%s?action=tools_viewsNavigator&content=%s' % (sysaddon, content)
				return control.execute('Container.Update(%s)' % url)
			title = getLS(32059)
			url = '%s?action=tools_addView&content=%s' % (sysaddon, content)
	...
	def accountCheck(self):
		if not self.traktCredentials and not self.imdbCredentials and not self.mdblCredentials:
			control.hide()
	...
	def addDirectoryItem(self, name, query, poster, icon, context=None, queue=False, isAction=True, isFolder=True, isPlayable=False, isSearch=False, table=''):
		try:
			...
			url = 'plugin://plugin.video.dradis/?action=%s' % query if isAction else query
			...
			if context: cm.append((getLS(context[0]), 'RunPlugin(plugin://plugin.video.dradis/?action=%s)' % context[1]))
			if isSearch: cm.append(('Clear Search Phrase', 'RunPlugin(plugin://plugin.video.dradis/?action=cache_clearSearchPhrase&source=%s&name=%s)' % (table, quote_plus(name))))
