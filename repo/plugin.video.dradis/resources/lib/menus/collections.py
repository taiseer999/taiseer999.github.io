...
#		if not self.tmdb_key: self.tmdb_key = ''
	...
	def search_new(self):
		...
		control.closeAll()
		control.execute('ActivateWindow(Videos,plugin://plugin.video.dradis/?action=collections&url=%s,return)' % (quote_plus(url)))
...
#				nextMenu = '[COLOR skyblue]' + nextMenu + page + '[/COLOR]'
	...
	def addDirectoryItem(self, name, action, poster, icon, fanart=None, context=None, queue=False):
		try:
			...
			url = 'plugin://plugin.video.dradis/?action=%s' % action
			cm = []
