...
#		self.persons_link = 'https://www.imdb.com/search/name/?count=100&name='
		...
		self.trakttrendingrecent_link = 'https://api.trakt.tv/shows/trending?page=1&limit=%s&years=%s-%s' % (self.page_limit, int(self.today_year)-1, self.today_year)
		...
		self.trakttrendinganime_link = 'https://api.trakt.tv/shows/trending?genres=anime&limit=%s&page=1' % self.page_limit
		self.traktwatchedanime_link = 'https://api.trakt.tv/shows/watched/all?genres=anime&limit=%s&page=1' % self.page_limit
...
#		if not self.tmdb_key: self.tmdb_key = ''
		...
		self.tmdb_popular_link = 'https://api.themoviedb.org/3/tv/popular?api_key=%s&with_original_language=en&language=en-US&region=US&page=1'
		...
		self.tmdb_popularanime_link = 'https://api.themoviedb.org/3/discover/tv?api_key=%s&with_keywords=210024&sort_by=popularity.desc&page=1'
	...
	def search_new(self):
		...
		control.closeAll()
		control.execute('ActivateWindow(Videos,plugin://plugin.video.dradis/?action=tvshows&url=%s,return)' % (quote_plus(url)))
...
#		url = self.persons_link + quote_plus(q)
		...
		control.closeAll()
		control.execute('ActivateWindow(Videos,plugin://plugin.video.dradis/?action=tvPersons&url=%s,return)' % (quote_plus(url)))
...
#		self.addDirectory(self.list)
	...
	def genres(self, url):
		...
		for i in genres:
			if self.imdb_link in url:
				self.list.append({'content': 'genres', 'name': cleangenre.lang(i[0], self.lang), 'url': url % i[1] if i[2] else self.keyword_link % i[1], 'image': i[0] + '.jpg', 'icon': i[0] + '.png', 'action': 'tvshows'})
			...
			if self.trakt_link in url:
				self.list.append({'content': 'genres', 'name': cleangenre.lang(i[0], self.lang), 'url': f"{url}&genres={i[1]}", 'image': f"{i[0]}.jpg" if i[2] else '', 'icon': f"{i[0]}.png" if i[2] else '', 'action': 'tvshows'})
		self.addDirectory(self.list)
		return self.list
	...
	def years(self, url):
		...
		for i in range(int(year)-0, 1900, -1):
			...
			if self.trakt_link in url: self.list.append({'content': 'years', 'name': str(i), 'url': f"{url}&years={i}", 'image': 'years.png', 'icon': 'DefaultYear.png', 'action': 'tvshows'})
		self.addDirectory(self.list)
		return self.list
...
####-Context Menu and Overlays-####
				cm = []
				if self.mdbl_user:
					cm.append(('[COLOR yellow]MDBList Manager[/COLOR]', 'RunPlugin(%s?action=tools_mdblistManager&name=%s&imdb=%s&media_type=%s)' % (sysaddon, systitle, imdb, 'shows')))
				try:
				...
				if not is_widget: cm.append(('[B]Exit TV Shows List[/B]', 'Container.Refresh(%s?action=tvNavigator)' % sysaddon))
####################################
...
#				nextMenu = '[COLOR skyblue]' + nextMenu + page + '[/COLOR]'
				...
				elif u in self.imdb_link or u in self.trakt_link or u in self.mdblist_link:
...
#			nextMenu = '[COLOR skyblue]' + nextMenu + page + '[/COLOR]'
	...
	def mdbl_user_lists(self, url):
		...
		for item in items:
			try:
				...
				self.list.append({'name': label, 'url': list_url, 'list_owner': list_owner, 'list_name': list_name, 'list_id': list_id, 'context': list_url, 'next': next, 'image': 'mdblist.png', 'icon': 'DefaultVideoPlaylists.png', 'action': 'tvshows'})
			except:
				from resources.lib.modules import log_utils
				log_utils.error()
		return self.list
	...
	def mdblist_list(self, url):
		...
		for item in items:
			try:
				...
				values['tvdb'] = str(item.get('tvdb_id', '')) if item.get('tvdb_id') else ''
				self.list.append(values)
			except:
				from resources.lib.modules import log_utils
				log_utils.error()
	...
	def mdblWatchlist(self, url, create_directory=True):
		self.list = []
		def _sort(type='shows.watchlist'):
			try:
				attribute = int(getSetting('sort.%s.type' % type))
	...
	def rpdb_get(self, media_id):
		if self.rpdb_api_key and media_id:
			...
			else: id_type, media_id = 'tmdb', 'series-%s'
