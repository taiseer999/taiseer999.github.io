...
class Movies:
	def __init__(self, notifications=True):
		...
		self.today_year = (self.date_time).strftime('%Y')
		self.first_day_month = str(date(datetime.now().year, datetime.now().month, 1))
		self.last_day_month = str(date(datetime.now().year, 1 if datetime.now().month==12 else datetime.now().month+1, 1) - timedelta(days=1))
		...
		self.mdbl_user = getSetting('mdblist.token')
...
#		if not self.tmdb_key: self.tmdb_key = ''
		...
		self.rpdb_api_key = getSetting('rpdb.apikey')
		self.enable_rpdb = getSetting('enable.rpdb') == 'true'
		...
		self.tmdb_digital_link = 'https://api.themoviedb.org/3/discover/movie?api_key=%s&language=en-US&region=US&sort_by=popular.desc&release_date.gte=%s&release_date.lte=%s&with_release_type=4|5&page=1'% ('%s', self.first_day_month, self.last_day_month)
		self.tmdb_popularanime_link = 'https://api.themoviedb.org/3/discover/movie?api_key=%s&with_keywords=210024&sort_by=popularity.desc&page=1'
...
#		self.persons_link = 'https://www.imdb.com/search/name/?count=100&name='
		...
		self.trakttrendingrecent_link = 'https://api.trakt.tv/movies/trending?limit=%s&page=1&years=%s-%s' % (self.page_limit, int(self.today_year)-1, self.today_year)
		...
		self.trakt_dvd_link = 'https://api.trakt.tv/calendars/all/dvd/%s-%s-%s/33' % (self.today_year, (self.date_time).strftime('%m'), '01')
		self.trakttrendinganime_link = 'https://api.trakt.tv/movies/trending?genres=anime&limit=100&page=1'
		self.traktwatchedanime_link = 'https://api.trakt.tv/movies/watched/all?genres=anime&limit=%s&page=1' % self.page_limit

		self.mdblist_link = 'https://api.mdblist.com'
		self.mdblistlist_link = 'https://api.mdblist.com/lists/%s/items?page=1'
		self.mdblistlists_link = 'https://api.mdblist.com/lists/user'
		self.mdblwatchlist_link = 'https://api.mdblist.com/watchlist/items?&page=1'
		self.mdbltop_link = 'https://api.mdblist.com/lists/top'
	...
	def get(self, url, idx=True, create_directory=True):
		...
		try:
			...
			if u in self.trakt_link and '/users/' in url:
				...
				if idx: self.worker()
			elif u in self.mdblist_link:
				if '/watchlist/' in url: return self.mdblWatchlist(url)
				if '/top' in url: return self.mdbl_top_lists(url)
				self.list = self.mdblist_list(url)
				if idx: self.worker()
	...
	def search_new(self):
		...
		control.closeAll()
		control.execute('ActivateWindow(Videos,plugin://plugin.video.dradis/?action=movies&url=%s,return)' % (quote_plus(url)))
...
#		url = self.persons_link + quote_plus(q)
		...
		control.closeAll()
		control.execute('ActivateWindow(Videos,plugin://plugin.video.dradis/?action=moviePersons&url=%s,return)' % (quote_plus(url)))
...
#		self.addDirectory(self.list)
	...
	def genres(self, url):
		...
		for i in genres:
			if self.imdb_link in url:
				self.list.append({'content': 'genres', 'name': cleangenre.lang(i[0], self.lang), 'url': url % i[1] if i[2] else self.keyword_link % i[1], 'image': i[0] + '.jpg', 'icon': i[0] + '.png', 'action': 'movies'})
			...
			if self.trakt_link in url:
				self.list.append({'content': 'genres', 'name': cleangenre.lang(i[0], self.lang), 'url': f"{url}&genres={i[1]}", 'image': f"{i[0]}.jpg" if i[2] else '', 'icon': f"{i[0]}.png" if i[2] else '', 'action': 'movies'})
		self.addDirectory(self.list)
		return self.list
	...
	def years(self, url):
		...
		for i in range(int(year)-0, 1900, -1):
			...
			if self.trakt_link in url: self.list.append({'content': 'years', 'name': str(i), 'url': f"{url}&years={i}", 'image': 'years.png', 'icon': 'DefaultYear.png', 'action': 'movies'})
		self.addDirectory(self.list)
		return self.list
	...
	def userlists(self):
		...
		try:
			if not self.mdbl_user: raise Exception()
			self.list = []
			lists = self.mdbl_user_lists(self.mdblistlists_link)
			userlists += lists
		except: pass
		try:
	...
	def trakt_public_list(self, url):
		...
		for item in items:
			try:
				...
				list_count = list_item.get('item_count', '?')
				...
				label = '%s - [COLOR %s]%s[/COLOR] - (%s)' % (list_name, self.highlight_color, list_owner, list_count)
	...
	def worker(self):
		try:
			...
			for r in range(0, total, int(self.page_limit)):
				threads = []
				append = threads.append
				for i in range(r, r + int(self.page_limit)):
...
#################################
	...
	def movieDirectory(self, items, unfinished=False, next=True):
		...
		yt_status = control.condVisibility('System.HasAddon(plugin.video.youtube)')
		...
		nextMenu, clearSourcesMenu = getLS(32053), getLS(32611) % 'red'
		...
		for i in items:
			try:
				...
				if self.enable_rpdb:
					rpdb_art = self.rpdb_get(imdb or tmdb)
					poster = rpdb_art.get('rpdb') or poster
					meta.update(rpdb_art)
				fanart = ''
				if settingFanart:
...
####-Context Menu and Overlays-####
				cm = []
				if self.mdbl_user:
					cm.append(('[COLOR yellow]MDBList Manager[/COLOR]', 'RunPlugin(%s?action=tools_mdblistManager&name=%s&imdb=%s&media_type=%s)' % (sysaddon, sysname, imdb, 'movies')))
				try:
				...
				if trailer and 'video_id' in trailer and yt_status: cm.append(('Trailer', f"RunPlugin({trailer})"))
				...
				if not is_widget: cm.append(('[B]Exit Movies List[/B]', 'Container.Refresh(%s?action=movieNavigator)' % sysaddon))
####################################
...
#				nextMenu = '[COLOR skyblue]' + nextMenu + page + '[/COLOR]'
				...
				elif u not in self.tmdb_link: url = '%s?action=moviePage&url=%s' % (sysaddon, quote_plus(url))
		...
		for i in items:
			try:
				...
				if content == 'genres':
					...
					poster = control.joinPath(control.genrePosterPath(), i['image']) if i['icon'] else addonThumb
				else:
					icon = i['icon']
				...
				cm.append(('[COLOR red]Dradis Settings[/COLOR]', 'RunPlugin(%s?action=tools_openSettings)' % sysaddon))
...
#			nextMenu = '[COLOR skyblue]' + nextMenu + page + '[/COLOR]'
	...
	def mdbl_user_lists(self, url):
		from resources.lib.indexers import mdblist
		items = []
		try:
			result = cache.get(mdblist.call_mdblist, 0.5, url)
			items = result
		except:
			from resources.lib.modules import log_utils
			log_utils.error()
		for item in items:
			try:
				if item['mediatype'] == 'show': continue
				list_name = item['name']
				list_owner = item['user_name']
				list_id = item['id']
				list_count = item['items']
				list_url = self.mdblistlist_link % list_id
				next = ''
				if '/top' in url: label = '%s - [COLOR %s]%s[/COLOR] - (%s)' % (list_name, self.highlight_color, list_owner, list_count)
				else: label = '%s - [COLOR %s]%s[/COLOR]' % (list_name, self.highlight_color, list_owner)
				self.list.append({'name': label, 'url': list_url, 'list_owner': list_owner, 'list_name': list_name, 'list_id': list_id, 'context': list_url, 'next': next, 'image': 'mdblist.png', 'icon': 'DefaultVideoPlaylists.png', 'action': 'movies'})
			except:
				from resources.lib.modules import log_utils
				log_utils.error()
		return self.list

	def mdblist_list(self, url):
		from resources.lib.indexers import mdblist
		self.list = []
		q = dict(parse_qsl(urlsplit(url).query))
		index = int(q['page']) - 1
		...
		for item in items:
			try:
				...
				values['year'] = item['release_year']
				values['imdb'] = str(item.get('imdb_id', '')) if item.get('imdb_id') else ''
	...
	def mdblWatchlist(self, url, create_directory=True):
		self.list = []
		def _sort(type='movies.watchlist'):
			try:
				attribute = int(getSetting('sort.%s.type' % type))
				reverse = 'desc' if int(getSetting('sort.%s.order' % type)) == 1 else 'asc'
				if attribute == 0: params, reverse = '', '' # Sorting Order is not enabled when sort method is "Default"
				if attribute > 0:
					if   attribute == 1: params = '&sort=title&order=%s' % reverse
					elif attribute == 2: params = '&sort=imdbrating&order=%s' % reverse
					elif attribute == 3: params = '&sort=imdbvotes&order=%s' % reverse
					elif attribute == 4: params = '&sort=released&order=%s' % reverse
					elif attribute == 5: params = '&sort=added&order=%s' % reverse
					elif attribute == 6: params = '&sort=last_air_date&order=%s' % reverse
				elif reverse:
					params = '&order=%s' % reverse
				return params
			except:
				from resources.lib.modules import log_utils
				log_utils.error()
		try:
			self.list = self.mdblist_list(url + _sort())
			self.worker()
			if self.list is None: self.list = []
			if create_directory: self.movieDirectory(self.list)
			return self.list
	...
	def mdbl_top_lists(self, url, create_directory=True):
		self.list = []
		try:
			if not self.mdbl_user: raise Exception()
			self.list = self.mdbl_user_lists(url)
		except: pass
		if self.list is None: self.list = []
		if create_directory: self.addDirectory(self.list, queue=True)
		return self.list

	def rpdb_get(self, media_id):
		if self.rpdb_api_key and media_id:
			if media_id.startswith('tt'): id_type = 'imdb'
			else: id_type, media_id = 'tmdb', 'movie-%s'
			url = 'https://api.ratingposterdb.com/%s/%s/poster-default/%s.jpg'
			rpdb_data = {'rpdb': url % (self.rpdb_api_key, id_type, media_id)}
		else: rpdb_data = {'rpdb': ''}
		return rpdb_data
