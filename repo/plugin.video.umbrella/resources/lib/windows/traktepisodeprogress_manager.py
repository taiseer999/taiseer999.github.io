			...
			if action in self.selection_actions:
				...
				if focus_id == 2060: # listItems
					...
					if chosen_listitem.getProperty('umbrella.isSelected') == 'true':
						...
						season = chosen_listitem.getProperty('umbrella.season')
						episode = chosen_listitem.getProperty('umbrella.episode')
					...
					for item in self.item_list:
						...
						self.selected_items.append({'imdb': item.getProperty('umbrella.imdb'), 'tvdb': item.getProperty('umbrella.tvdb'),
							'season': item.getProperty('umbrella.season'), 'episode': item.getProperty('umbrella.episode')})
		...
		def builder():
			for count, item in enumerate(self.results, 1):
				try:
					...
					listitem.setProperty('umbrella.tvshowtitle', tvshowtitle)
					...
					listitem.setProperty('umbrella.premiered', new_date)
					season = str(item.get('season'))
					listitem.setProperty('umbrella.season', season)
					episode = str(item.get('episode'))
					listitem.setProperty('umbrella.episode', episode)
					label = '%s  -  %sx%s' % (tvshowtitle, season, episode.zfill(2))
					listitem.setProperty('umbrella.label', label)
					...
					listitem.setProperty('umbrella.genre', str(item.get('genre', '')))
