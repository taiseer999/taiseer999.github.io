...
class myAddon(t1mAddon):
  ...
  def getAddonMenu(self,url,ilist):
    ...
    for key,b in a['containerMenuHash'].items():
        url = b['slug']
        name = b['title']
    ...
    searchInfoList = {'Title': self.localLang(30010), 'Plot': ''}
    ilist = self.addMenuItem(searchInfoList['Title'], 'GM', ilist, 'https://tubitv.com/oz/search/', self.addonIcon, self.addonFanart, searchInfoList, isFolder=True)
    return(ilist)
  ...
  def getAddonVideo(self,url):
   ...
   # xbmc.log(msg=str(a), level=xbmc.LOGINFO)
   url = a.get("url", None)
   if url != None:
       subs = a["subtitles"]
       for sub in subs:
