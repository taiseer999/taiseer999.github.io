...
def play_youtube(video_id, mode=None):
    ...
    if mode == YTMode.APK:
        ...
        return TrailerItem(path='plugin://{}/play/?video_id={}'.format(YOTUBE_PLUGIN_ID, video_id))
    elif mode == YTMode.TUBED_PLUGIN:
        assert_not_redirect(TUBED_PLUGIN_ID)
        return TrailerItem(path='plugin://{}/?mode=play&video_id={}'.format(TUBED_PLUGIN_ID, video_id))
