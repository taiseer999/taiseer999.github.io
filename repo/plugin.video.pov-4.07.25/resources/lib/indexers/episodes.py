...
class Episodes:
	...
	def run(self):
		try:
			...
			self.sort_type, self.category = 'unsorted', ls(params_get('name'))
		...
		kodi_utils.set_view_mode(self.view_type, self.content_type)
	...
	def build_episode_content(self, item_position, ep_data):
		try:
			...
			if self.list_type == 'next_episode_trakt':
				cm_append((nextep_manager_str, container_update % build_url({'mode': 'build_next_episode_manager'})))
...
#			listitem.setContentLookup(False)
	...
	def worker(self):
		...
		if self.is_widget: container_update = 'ActivateWindow(Videos,%s,return)'
		if self.list_type_starts_with('next_episode'):
