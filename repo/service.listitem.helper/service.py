...
class Main:
    ...
    def run_service(self):
        while not self.monitor.abortRequested():
            ...
            tmpCheckCondition = not IsScrolling and IsMovieContent and (IsVideoLibraryView or IsMovieInfoDialogView) and not IsAddon
            ...
            if tmpCheckCondition:
                ...
                if (self.selecteditem and self.selecteditem != self.previousitem):
                    ...
                    if (xbmc.getInfoLabel("ListItem.DBTYPE") == 'movie' and xbmc.getInfoLabel("ListItem.IMDBNumber") and xbmc.getInfoLabel("ListItem.DBID") and not xbmc.getCondVisibility("ListItem.IsFolder")):
                ...
                if tmpCheckCondition:
                    ...
                    if (self.selecteditem and self.selecteditem != self.previousitem):
                        ...
                        if (xbmc.getInfoLabel("Container("+my_container_id+").ListItem.IMDBNumber") and xbmc.getInfoLabel("Container("+my_container_id+").ListItem.DBID") and xbmc.getInfoLabel("Container("+my_container_id+").ListItem.DBTYPE") == 'movie' and not xbmc.getCondVisibility("Container("+my_container_id+").ListItem.IsFolder")):
                            ...
                            # Validate self.imdbnumber_corrected. ListItem.IMDBNumber sometimes wrong (some scraper wrongly put their own id into this property)
                            # tt, nm, co, ev, ch, ni
                            #isValidIMDBNumber = xbmc.getCondVisibility("String.StartsWith(xbmc.getInfoLabel("ListItem.IMDBNumber"),tt)") or xbmc.getCondVisibility("String.StartsWith(xbmc.getInfoLabel("ListItem.IMDBNumber"),nm)") or xbmc.getCondVisibility("String.StartsWith(xbmc.getInfoLabel("ListItem.IMDBNumber"),co)") or xbmc.getCondVisibility("String.StartsWith(xbmc.getInfoLabel("ListItem.IMDBNumber"),ev)") or xbmc.getCondVisibility("String.StartsWith(xbmc.getInfoLabel("ListItem.IMDBNumber"),ch)") or xbmc.getCondVisibility("String.StartsWith(xbmc.getInfoLabel("ListItem.IMDBNumber"),ni)")
                            isValidIMDBNumber = self.imdbnumber_corrected.startswith('tt') or self.imdbnumber_corrected.startswith('nm') or self.imdbnumber_corrected.startswith('co') or self.imdbnumber_corrected.startswith('ev') or self.imdbnumber_corrected.startswith('ch') or self.imdbnumber_corrected.startswith('ni')
    ...
    def set_helper_values(self):
        ...
        for x in range(1, 51):
            # clear all
            xbmc.executebuiltin('ClearProperty(ListItemHelper.studio.'+str(x)+',home)')
            xbmc.executebuiltin('ClearProperty(ListItemHelper.genre.'+str(x)+',home)')
            xbmc.executebuiltin('ClearProperty(ListItemHelper.writer.'+str(x)+',home)')
            xbmc.executebuiltin('ClearProperty(ListItemHelper.director.'+str(x)+',home)')
        
        if self.studio:
            splitString = self.studio.split(self.videolibrary_itemseparator)
            if splitString:
                indexNo = 1
        ...
        if self.genre:
            splitString = self.genre.split(self.videolibrary_itemseparator)
            if splitString:
                indexNo = 1
                for item in splitString:
        ...
        if self.writer:
            splitString = self.writer.split(self.videolibrary_itemseparator)
            if splitString:
                indexNo = 1
                for item in splitString:
        ...
        if self.director:
            splitString = self.director.split(self.videolibrary_itemseparator)
            if splitString:
                indexNo = 1
                for item in splitString:
        ...
        # MDBList
        if self.imdbnumber_corrected_isvalid:
            self.mdblist_result = self.get_mdblist_details(self.imdbnumber_corrected)
        
        if (self.imdbnumber_corrected_isvalid and self.mdblist_result):
        ...
        if (self.imdbnumber_corrected_isvalid and self.omdb_result) :
        ...
        if (self.imdbnumber_corrected_isvalid and self.tmdb_result) :
