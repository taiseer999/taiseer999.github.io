...
BLUR_CONTAINER = 100000#xbmc.getInfoLabel('Skin.String(BlurContainer)') or 100000
BLUR_RADIUS = 4#xbmc.getInfoLabel('Skin.String(BlurRadius)') or ADDON.getSetting('blur_radius')
BLUR_SATURATION = '1.0'#xbmc.getInfoLabel('Skin.String(BlurSaturation)') or '1.0'
OLD_IMAGE = ''
...
'''
class ImageBlur():
    def __init__(self,prop='listitem',file=None,radius=None,saturation=None,force=None):
        global OLD_IMAGE
        ...
        self.force = True if force is not None else False

        if self.image:
            if self.force or self.image != OLD_IMAGE:
    ...
    def blur(self):
        blurimgsize = int(400)
        filename = md5hash(self.image) + '-' + str(blurimgsize) + 'x' + str(blurimgsize) + '-'  + str(self.radius) + '-' + str(self.saturation) + '.png'
        targetfile = os.path.join(ADDON_DATA_IMG_PATH, filename)

        try:
            blur_img_now = False
            if not self.force and xbmcvfs.exists(targetfile):
                
                f = xbmcvfs.File(targetfile)
                if f.size() <= 0:
                    # Existing blurred file empty - redo it
                    blur_img_now = True
                    log('Image error: Existing blurred file empty (0 bytes) - redo it', DEBUG)
                f.close()
            ...
            if blur_img_now:
                img = _openimage(self.image,ADDON_DATA_IMG_PATH,filename,self.force)
                img.thumbnail((blurimgsize, blurimgsize), Image.LANCZOS)
                img = img.convert('RGB')
    ...
    def create_thumb(self):
        ...
        try:
            ...
            for poster in self.temp_files:
                image = ImageOps.fit(poster, (size), method=Image.LANCZOS, bleed=0.0, centering=(0.5, 0.5))
...
'''
def _openimage(image,targetpath,filename,force=False):
    ...
    for i in range(1, 4):
        try:
            ...
            for cache in cached_files:
                if not force and xbmcvfs.exists(cache):
                    try:
            ...
            '''
            if not force and xbmc.skinHasImage(image):
