...
def addonFanart():
	user_background = setting('user_background')
	if user_background: return user_background
	theme = appearance()
	art = artPath()
...
def refresh_playAction(): # for infinity global CM play actions
	...
	homeWindow.setProperty('infinity.autoPlayTv.enabled', autoPlayTV)
	autoPlayMovie = 'true' if setting('play.mode.movie') == '1' else '0'
	homeWindow.setProperty('infinity.autoPlayMovie.enabled', autoPlayMovie)
