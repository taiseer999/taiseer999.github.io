...
def router(argv2):
	...
	if action is None:
		...
		isUpdate = control.homeWindow.getProperty('infinity.updated')
		if isUpdate == 'true':
			control.execute('RunPlugin(plugin://plugin.video.infinity/?action=tools_cleanSettings)')
			control.homeWindow.clearProperty('infinity.updated')
			from resources.lib.modules import changelog
			changelog.get('Infinity')
	...
	elif action == 'moviesBasedOn':
		from resources.lib.menus import movies
		movies.Movies().traktBasedOn(url, folderName=folderName)
	...
	#added for tmdb based searches for movies.
		...
		from resources.lib.menus import navigator
		navigator.Navigator().trakt_genre(mediatype=mediatype, genre=genre, url=url, folderName=folderName)
	elif action == 'trakt_genre' and mediatype == 'Movies':
		from resources.lib.menus import movies
		movies.Movies().trakt_genre_list(listType=listType, genre=genre, url=url, folderName=folderName)
	elif action == 'trakt_genre_decade' and mediatype == 'Movies':
		from resources.lib.menus import movies
		...
		if name:
			...
			from resources.lib.menus import movies
			movies.Movies().persons(url, folderName=folderName)
	elif action == 'dvdreleaselistMovies':
		from resources.lib.menus import movies
		movies.Movies().getDvdReleaseList(url, folderName='DVD Releases')
		...
		from resources.lib.menus import movies
		movies.Movies().favouriteMovies(folderName=folderName)
	...
	elif action == 'tvBasedOn':
		from resources.lib.menus import tvshows
		tvshows.TVshows().traktBasedOn(url, folderName=folderName)
	...
	elif action == 'trakt_genre' and mediatype == 'TVShows':
		from resources.lib.menus import tvshows
		tvshows.TVshows().trakt_genre_list(listType=listType, genre=genre, url=url, folderName=folderName)
	elif action == 'trakt_genre_decade' and mediatype == 'TVShows':
		from resources.lib.menus import tvshows
	...
	elif action == 'traktProgress':
		from resources.lib.menus import tvshows
		tvshows.TVshows().traktProgress(url, mediatype=mediatype, folderName=folderName)
	elif action == 'shows_favourites':
		from resources.lib.menus import tvshows
		tvshows.TVshows().favouriteTVShows(folderName=folderName)
	...
	elif action == 'episodes_favourites':
		from resources.lib.menus import episodes
	...
	####################################################
		...
		if action == 'ad_ServiceNavigator':
			...
			if query: control.openSettings(query)
...
#			newsinfo.news()
			from resources.lib.indexers import cnio
			cnio.news()
		...
		elif action == 'tools_contextInfinitySettings':
			control.openSettings('0.0', 'context.infinity')
			...
			elif cleared: control.notification(message='Infinity Log File Successfully Cleared')
			else: control.notification(message='Error clearing Infinity Log File, see kodi.log for more info')
		...
		elif action == 'tools_infinityExternalProvider':
			from resources.lib.modules import tools
			tools.external_providers()
		elif action == 'tools_infinityExternalProviderSettings':
			module = control.setting('external_provider.module')
			if module: control.openSettings('0.0', module)
	...
	####################################################
		...
		if action == 'play_Item':
			...
			if caller == 'realdebrid':
				...
				from resources.lib.debrid import torbox
				if params.get('type') == 'unrestrict': control.player.play(torbox.TorBox().unrestrict_link(url.replace(' ', '%20')))
				else: control.player.play(url.replace(' ', '%20'))
			else:
			...
			for i in items:
				...
				url = 'plugin://plugin.video.infinity/?action=play_Item&title=%s&year=%s&imdb=%s&tmdb=%s&tvdb=%s&season=%s&episode=%s&tvshowtitle=%s&premiered=%s&meta=%s&select=1' % (
			...
			if rtype == 'movie':
				...
				r = 'plugin://plugin.video.infinity/?action=play_Item'
			elif rtype == 'episode':
				from resources.lib.menus import episodes
				...
				r = 'plugin://plugin.video.infinity/?action=play_Random&rtype=episode'
			elif rtype == 'show':
				from resources.lib.menus import tvshows
				rlist = tvshows.TVshows().get(url, create_directory=False)
				r = 'plugin://plugin.video.infinity/?action=play_Random&rtype=season'
	...
	####################################################
		...
		elif action == 'cache_clearKodiBookmark': # context.infinity action call only
	...
	####################################################
		...
		from resources.lib.debrid import plex
		plex.Plex().auth()
	...
	elif action == 'favouriteNavigator':
		from resources.lib.menus import navigator
	...
	####################################################
		...
		control.sleep(200)
		control.openSettings('0.2', 'plugin.video.infinity')
	...
	elif action == 'simkl_Authorize':
		from resources.lib.indexers import simkl
		simkl.SIMKL().auth()
		if query: control.openSettings(query)
	elif action == 'simkl_Revoke':
		from resources.lib.indexers import simkl
		simkl.SIMKL().revoke()
		if query: control.openSettings(query)

	elif action == 'openSubsTest':
		from resources.lib.indexers import opensubs
		opensubs.OpenSubtitles().getAccountStatus()
	elif action == 'openSubsAuth':
		from resources.lib.indexers import opensubs
		opensubs.OpenSubtitles().auth()
		if query: control.openSettings(query)
	elif action == 'openSubsRevoke':
		from resources.lib.indexers import opensubs
		opensubs.OpenSubtitles().revoke()
		if query: control.openSettings(query)

	elif action == 'cometConfigure':
		from resources.lib.modules.hosted import Comet
		Comet().configure()
