...
def router(argv2):
	try:
#		if not argv2 and 'play_Item' in control.infoLabel('ListItem.FolderPath'):
#			url = control.infoLabel('ListItem.FolderPath')
#			argv2 = url.replace('plugin://plugin.video.infinity/', '')
	...
	#added for tmdb based searches for movies.
		...
		from resources.lib.menus import movies
		movies.Movies().getDvdReleaseList(folderName='DVD Releases')
		...
		from resources.lib.menus import movies
		movies.Movies().recommendedFromLibrary(folderName=folderName)
...
#			newsinfo.news()
		...
		if action == 'tools_openSubsRevoke':
			...
			if query == 'EasyNews':
				...
				control.sleep(100)
				control.openSettings('11.3', 'plugin.video.infinity')
				...
				control.syncAccounts()
				control.sleep(100)
				control.openSettings('11.4', 'plugin.video.infinity')
			elif query == 'Plex':
				...
				control.syncAccounts()
				control.sleep(100)
				control.openSettings('11.7', 'plugin.video.infinity')
			else:
