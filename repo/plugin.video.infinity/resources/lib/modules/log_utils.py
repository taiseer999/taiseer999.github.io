...
DEBUGPREFIX = '[ plugin.video.infinity: %s ]'
...
def log(msg, caller=None, level=LOGINFO):
	...
	try:
		...
		if debug_location == '1':
			log_file = joinPath(LOGPATH, 'infinity.log')
		...
		traceback.print_exc()
		import xbmc
		xbmc.log('[ plugin.video.infinity ] log_utils.log() Logging Failure: %s' % (e), LOGERROR)
...
def error(message=None, exception=True):
	try:
		...
		import xbmc
		xbmc.log('[ plugin.video.infinity ] log_utils.error() Logging Failure: %s' % (e), LOGERROR)
...
def clear_logFile():
	...
	try:
		...
		import xbmc
		xbmc.log('[ plugin.video.infinity ] log_utils.clear_logFile() Failure: %s' % (e), LOGERROR)
		cleared = False
	return cleared
...
def upload_LogFile(name):
	...
	try:
		...
		text = f.read()
		f.close()
		UserAgent = 'Infinity %s' % addonVersion('plugin.video.infinity')
		...
		if 'key' in response.json():
			...
			select = selectDialog([i[0] for i in list], lang(32196) if name.lower() == 'infinity' else lang(32199))
