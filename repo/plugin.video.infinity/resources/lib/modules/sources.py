# -*- coding: utf-8 -*-
"""
	OneMoar Add-on Updated 10-3-22
"""
...
class Sources:
	...
	def play(self, title, year, imdb, tmdb, tvdb, season, episode, tvshowtitle, premiered, meta, select, rescrape=None):
		...
		try:
			...
			preResolved_nextUrl = playerWindow.getProperty('infinity.preResolved_nextUrl')
...
##################
			...
			if select is None:
				...
				elif episode is not None: select = getSetting('play.mode.tv')
				else: select = getSetting('play.mode.movie')
	...
	def sourceSelect(self, title, items, uncached_items, meta):
		...
		try:
			...
			if action == 'play_Item' and self.uncached_chosen != True:
				return self.playItem(title, items, chosen_source.getProperty('infinity.source_dict'), self.meta)
			else:
	...
	def playItem(self, title, items, chosen_source, meta):
		try:
			...
			try:
				...
				elif getSetting('progress.dialog') in ('2', '3'):
					progressDialog = self.window or self.getSourceProgress(title, meta)
				else: raise Exception()
			except:
				homeWindow.clearProperty('infinity.source_progress_is_alive')
			...
			for i in range(len(resolve_items)):
				try:
					...
					if getSetting('progress.dialog') == '0':
						...
						label = '[B][COLOR %s]%s[CR]%s[CR]%s[CR]%02d - %s[/COLOR][/B]' % (self.highlight_color, src_provider.upper(), resolve_items[i]['provider'].upper(), resolve_items[i]['info'], resolve_index, resolve_items[i]['name'][:30])
					else:
						label = '[B][COLOR %s]%s[CR]%02d - %s[CR]%s[/COLOR][/B]' % (self.highlight_color, src_provider.upper(), resolve_index, resolve_items[i]['name'], str(round(resolve_items[i]['size'], 2)) + ' GB') # using "[CR]" has some weird delay with progressDialog.update() at times
					control.sleep(100)
					try:
						if progressDialog.iscanceled(): break
#						progressDialog.update(int((100 / float(len(resolve_items))) * i), label)
						progressDialog.update(int(100 - (100 * (1 + i) / len(resolve_items))), label)
					except: 
#							progressDialog.update(int((100 / float(len(resolve_items))) * i), '[COLOR %s]Resolving...[/COLOR]%s' % (self.highlight_color, resolve_items[i]['name']))
							progressDialog.update(int(100 - (100 * (1 + i) / len(resolve_items))), '[COLOR %s]Resolving...[/COLOR]%s' % (self.highlight_color, resolve_items[i]['name']))
					...
					if not any(x in self.url.lower() for x in video_extensions) and 'plex.direct:' not in self.url and 'torbox' not in self.url:
						...
						continue
					if homeWindow.getProperty('infinity.source_progress_is_alive') != 'true':
		...
		try:
			...
			try:
				if getSetting('progress.dialog') == '0':
					...
					debrid_message = '[COLOR %s][B]Please wait...[CR]Checking Debrid[/B][/COLOR]' % self.highlight_color
				elif getSetting('progress.dialog') in ('2', '3'):
					meta = self.meta or {'title': title, 'year': year, 'imdb': imdb, 'tvdb': tvdb, 'season': season, 'episode': episode, 'tvshowtitle': tvshowtitle}
					progressDialog = self.getSourceProgress(title, meta)
					debrid_message = '[COLOR %s][B]Please wait...[CR]Checking Debrid[/B][/COLOR]' % self.highlight_color
				else: raise Exception()
			except:
				homeWindow.clearProperty('infinity.source_progress_is_alive')
				progressDialog = control.progressDialogBG
				progressDialog.create(header, '')
				debrid_message = '[COLOR %s][B]Please wait...  Checking Debrid[/B][/COLOR]' % self.highlight_color
			self.prepareSources()
			sourceDict = self.sourceDict
...
#					percent = int((current_progress / float(timeout)) * 100)
					...
					else: progressDialog.update(max(1, percent), line3)
				...
				try:
					...
					if url:
						...
						if player_hasVideo: # do not setPropery if user stops playback quickly because "onPlayBackStopped" is already called and won't be able to clear it.
							playerWindow.setProperty('infinity.preResolved_nextUrl', url)
	...
	def sourcesFilter(self):
		...
		for d in self.debrid_resolvers:
			...
			if d.name == 'TorBox' and getSetting('torbox.enable') == 'true':
				try:
					valid_hoster = []
					threads.append(Thread(name=d.name.upper(), target=checkStatus, args=(self.tb_cache_chk_list, d.name, valid_hoster)))
				except: log_utils.error()
		if threads:
			[i.start() for i in threads]
	...
	def sourcesAutoPlay(self, items):
		...
		try:
			if getSetting('progress.dialog') == '0':
				...
				progressDialog = self.getSourceProgress(self.title, self.meta)
			else: raise Exception()
		except:
			homeWindow.clearProperty('infinity.source_progress_is_alive')
			progressDialog = control.progressDialogBG
...
#					progressDialog.update(int((100 / float(len(items))) * i), label)
					progressDialog.update(int(100 - (100 * (1 + i) / len(items))), label)
#				except: progressDialog.update(int((100 / float(len(items))) * i), '[COLOR %s]Resolving...[/COLOR]%s' % (self.highlight_color, items[i]['name']))
				except: progressDialog.update(int(100 - (100 * (1 + i) / len(items))), '[COLOR %s]Resolving...[/COLOR]%s' % (self.highlight_color, items[i]['name']))
				try:
					if control.monitor.abortRequested(): return sysexit()
...
#					Sources(all_providers='true', rescrapeAll='true').play(title, year, imdb, tmdb, tvdb, season, episode, tvshowtitle, premiered, self.meta, select=select, rescrape='true')
	...
	def getConstants(self):
		self.metaProperty = 'plugin.video.infinity.container.meta'
		self.seasonProperty = 'plugin.video.infinity.container.season'
		self.episodeProperty = 'plugin.video.infinity.container.episode'
		self.titleProperty = 'plugin.video.infinity.container.title'
		self.imdbProperty = 'plugin.video.infinity.container.imdb'
		self.tmdbProperty = 'plugin.video.infinity.container.tmdb'
		self.tvdbProperty = 'plugin.video.infinity.container.tvdb'
		self.labelProperty = 'plugin.video.infinity.container.label'
		if getSetting('provider.external.enabled') == 'true':
			try:
				module = getSetting('external_provider.module', '')
				syspath.append(control.transPath('special://home/addons/%s/lib' % module))
...
#				count += 1
			[i.update({'source': f"unchecked{' (pack)' if 'package' in i else ''}"}) for i in torrent_List]
			return torrent_List
		except: log_utils.error()
...
#					else: i.update({'source': 'uncached torrent'})
	...
	def getSourceProgress(self, title, meta):
		from resources.lib.windows.source_progress import SourceProgress
		self.window = SourceProgress('source_progress.xml', control.addonPath(control.addonId()), title=title, meta=meta)
		self.window.create()
		return self.window
