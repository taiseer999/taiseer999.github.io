...
class source:
	...
	def __init__(self):
		services = {
			'0': ('realdebrid', 'realdebridtoken'), '1': ('premiumize', 'premiumizetoken'), '2': ('alldebrid', 'alldebridtoken'), '3': ('', '')
		}
		debrid = getSetting('knightcrawler.debrid', '3')
		debrid, token = services[debrid]
		token = getSetting(token, '') if debrid else ''
		params = f"/{debrid}={token}" if token else ''
		self.language = ['en']
		self.base_link = "https://knightcrawler.elfhosted.com"
	...
	def sources(self, data, hostDict):
		...
		for file in files:
			try:
				...
				name = scrape_utils.clean_name(file_title[0] if len(file_title) < 3 else file_title[1])
...
#				except: seeders = 0
				...
				append({'provider': 'knightcrawler', 'source': 'torrent', 'seeders': 0, 'hash': hash, 'name': name, 'name_info': name_info, 'quality': quality,
...
#				except: seeders = 0
				...
				item = {'provider': 'knightcrawler', 'source': 'torrent', 'seeders': 0, 'hash': hash, 'name': name, 'name_info': name_info, 'quality': quality,
