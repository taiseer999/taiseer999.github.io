...
from resources.lib.indexers.simkl import SIMKL as simkl
...
#			self.tmdb_key = ''
	...
	def get(self, url, idx=True, create_directory=True, folderName=''):
		...
		try:
			...
			elif u in self.tmdb_link: # and url != 'tmdbrecentday' and url != 'tmdbrecentweek':
	...
	def search_new(self):
		...
		control.closeAll()
		control.execute('ActivateWindow(Videos,plugin://plugin.video.infinity/?action=tvshows&url=%s,return)' % (quote_plus(url)))
...
#################################
	...
	def tvshowDirectory(self, items, next=True, isProgress=False, isWatched=False, folderName='Infinity'):
		...
		homeWindow.setProperty('infinity.returnhome', returnHome)
...
####-Context Menu and Overlays-####
				...
				if not is_widget and page > 2:
					cm.append((exitMenu, 'Container.Refresh(%s?action=tvNavigator)' % sysaddon))
...
####################################
				...
				if is_widget: 
					item.setProperty('isInfinity_widget', 'true')
					if self.hide_watched_in_widget and str(control.infoLabel("Window.Property(xmlfile)")) != 'Custom_1114_Search.xml':
		...
		try:
			...
			if self.useContainerTitles:
				try: 
					...
					homeWindow.setProperty('infinity.tvrecent', str(getLS(40257)+' '+item.get('title')))
				except: pass
			next = ''
			for i in range(len(self.list)): self.list[i]['next'] = next
			self.worker()
		...
		try:
			...
			if self.useContainerTitles:
				try: 
					...
					homeWindow.setProperty('infinity.tvsimilar', str(getLS(40257)+' '+item.get('title')))
				except: pass
			next = ''
			for i in range(len(self.list)): self.list[i]['next'] = next
			self.worker()
