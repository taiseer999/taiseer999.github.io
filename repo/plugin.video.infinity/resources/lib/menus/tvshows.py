...
KODI_VERSION = control.KODI_VERSION
...
#		self.persons_link = 'https://www.imdb.com/search/name/?count=100&name='
		...
		self.persons_link = 'https://api.trakt.tv/people/%s/shows?limit=%s&page=1' % ('%s', self.search_page_limit)
		...
		self.trakttrending_recent_link = 'https://api.trakt.tv/shows/trending?page=1&limit=%s&years=%s' % (self.page_limit, '%s-%s' % (self.date_time.year - 1, self.date_time.year) if self.date_time.month < 6 else str(self.date_time.year))
...
#			self.tmdb_key = ''
		...
		self.tmdb_trendingday_link = 'https://api.themoviedb.org/3/trending/tv/day?api_key=%s&language=en-US&region=US&page=1'
		self.tmdb_trendingweek_link = 'https://api.themoviedb.org/3/trending/tv/week?api_key=%s&language=en-US&region=US&page=1'
		...
		self.search_tmdb_link = 'https://api.themoviedb.org/3/search/tv?api_key=%s&language=en-US&query=%s&region=US&page=1'
		...
		self.simkl_link = self.simkl_link = 'https://api.simkl.com'
		self.simkl_animetrending_link = 'https://api.simkl.com/anime/trending/week'
		self.simkl_animeweek_link = 'https://api.simkl.com/anime/genres/all/series/all-years/popular-this-week'
		self.simkl_animedate_link = 'https://api.simkl.com/anime/genres/all/series/all-years/release-date'
		self.simkl_animebest_link = 'https://api.simkl.com/anime/best/watched?type=tv'
		self.simkl_animevote_link = 'https://api.simkl.com/anime/best/voted?type=tv'
		self.simkl_animegenre_link = 'https://api.simkl.com/anime/genres/%s/series/all-years'
		self.simkl_onasweek_link = 'https://api.simkl.com/anime/genres/all/onas/all-years/popular-this-week'
		self.simkl_onasrank_link = 'https://api.simkl.com/anime/genres/all/onas/all-years/rank'
		self.simkl_onasdate_link = 'https://api.simkl.com/anime/genres/all/onas/all-years/release-date'
		...
		self.tmdb_trendingday_hours = int(getSetting('cache.tmdbtrendingday'))
		self.tmdb_trendingweek_hours = int(getSetting('cache.tmdbtrendingweek'))
		self.mdblist_hours = int(getSetting('cache.mdblist'))
		self.simkl_hours = int(getSetting('cache.simkl'))
		self.hide_watched_in_widget = getSetting('enable.infinityhidewatched') == 'true'
		self.hideCounts = getSetting('tvshows.episodecount') == 'true'
		self.useFullContext = getSetting('enable.infinitywidgetcontext') == 'true'
	...
	def get(self, url, idx=True, create_directory=True, folderName=''):
		...
		try:
			...
			if u in self.trakt_link and '/users/' in url:
				...
				if '/recommendations/' in url: random.shuffle(self.list)
				if idx: self.worker()
			elif u in self.simkl_link:
				from resources.lib.indexers.simkl import SIMKL as simkl_indexer
				self.list = cache.get(simkl_indexer().simkl_list, self.simkl_hours, url)
				if idx: self.worker()
	...
	def getTMDb(self, url, create_directory=True, folderName=''):
		...
		try:
			...
			if u in self.tmdb_link and 'trendingday' in url:
				self.list = cache.get(tmdb_indexer().tmdb_list, self.tmdb_trendingday_hours, url)
			elif u in self.tmdb_link and 'trendingweek' in url:
				self.list = cache.get(tmdb_indexer().tmdb_list, self.tmdb_trendingweek_hours, url)
			elif u in self.tmdb_link and '/list/' in url:
	...
	def collectionManager(self):
		try:
			...
			del window
			if selected_items:
				# refresh = 'plugin.video.infinity' in control.infoLabel('Container.PluginName')
	...
	def search_new(self):
		...
		try:
			...
			dbcur.close() ; dbcon.close()
		if self.traktCredentials and getSetting('searchtv.indexer') == '1':
			action, url = 'tvshows', self.search_link + quote_plus(q)
		else:
			action, url = 'tmdbTvshows', self.search_tmdb_link % ('%s', quote_plus(q))
		control.closeAll()
		control.execute('ActivateWindow(Videos,plugin://plugin.video.infinity/?action=%s&url=%s,return)' % (action, quote_plus(url)))

	def search_term(self, name):
		if name:
			if self.traktCredentials and getSetting('searchtv.indexer') == '1':
				target, url = self.get, self.search_link + quote_plus(name)
			else:
				target, url = self.getTMDb, self.search_tmdb_link % ('%s', quote_plus(name))
			target(url)
		else:
			return
...
#		url = self.persons_link + quote_plus(q)
		url = self.persons_link % quote_plus(trakt.slug(q))
		control.closeAll()
		control.execute('ActivateWindow(Videos,plugin://plugin.video.infinity/?action=tvPersons&url=%s,return)' % (quote_plus(url)))
...
#		else: self.list = cache.get(self.imdb_person_list, 1, url)
		self.list = cache.get(self.trakt_person_list, 96, url)
		if self.list is None: self.list = []
#		if self.list:
...
#		self.addDirectory(self.list, folderName=folderName)
	...
	def genres(self, url, folderName=''):
		...
		if 'trakt' in url: genres = [
			('Action', 'action', True), ('Adventure', 'adventure', True), ('Animation', 'animation', True),
			('Anime', 'anime', True), ('Biography', 'biography', True), ('Children', 'children', False), ('Comedy', 'comedy', True),
			('Crime', 'crime', True), ('Documentary', 'documentary', True), ('Donghua', 'donghua', False), ('Drama', 'drama', True),
			('Family', 'family', True), ('Fantasy', 'fantasy', True), ('Game Show', 'game-show', False),
			('History', 'history', True), ('Holiday', 'holiday', False), ('Home And Garden', 'home-and-garden', False),
			('Horror', 'horror', True), ('Mini Series', 'mini-series', False), ('Music', 'music', True),
			('Musical', 'musical', True), ('Mystery', 'mystery', True), ('News', 'news', True), ('Reality', 'reality', True),
			('Romance', 'romance', True), ('Science Fiction', 'science-fiction', True),
			('Soap', 'soap', False), ('Special Interest', 'special-interest', False), ('Sporting Event', 'sporting-event', False),
			('Superhero', 'superhero', False), ('Suspense', 'suspense', False), ('Talk-Show', 'talk-show', True),
			('Thriller', 'thriller', True), ('War', 'war', True), ('Western', 'western', True)]
		elif 'simkl' in url: genres = [
			('Anime', 'anime', False), ('Action', 'action', False), ('Adventure', 'adventure', False), ('Cars', 'cars', False), ('Comedy', 'comedy', False),
			('Dementia', 'dementia', False), ('Demons', 'demons', False), ('Drama', 'drama', False), ('Ecchi', 'ecchi', False), ('Fantasy', 'fantasy', False),
			('Game', 'game', False), ('Harem', 'harem', False), ('Historical', 'historical', False), ('Horror', 'horror', False), ('Josei', 'josei', False),
			('Kids', 'kids', False), ('Magic', 'magic', False), ('Martial Arts', 'martial-arts', False), ('Mecha', 'mecha', False), ('Military', 'military', False),
			('Music', 'music', False), ('Mystery', 'mystery', False), ('Parody', 'parody', False), ('Police', 'police', False), ('Psychological', 'psychological', False),
			('Romance', 'romance', False), ('Samurai', 'samurai', False), ('School', 'school', False), ('Sci-Fi', 'sci-fi', False), ('Seinen', 'seinen', False),
			('Shoujo', 'shoujo', False), ('Shoujo Ai', 'shoujo-ai', False), ('Shounen', 'shounen', False), ('Shounen Ai', 'shounen-ai', False),
			('Slice Of Life', 'slice-of-life', False), ('Space', 'space', False), ('Sports', 'sports', False), ('Super Power', 'super-power', False),
			('Supernatural', 'supernatural', False), ('Thriller', 'thriller', False), ('Vampire', 'vampire', False), ('Yaoi', 'yaoi', False), ('Yuri', 'yuri', False)]
		else: genres = [
		...
		for i in genres:
			if self.imdb_link in url:
				self.list.append({'content': 'genres', 'name': cleangenre.lang(i[0], self.lang), 'url': url % i[1] if i[2] else self.keyword_link % i[1], 'image': i[0] + '.jpg', 'icon': i[0] + '.png', 'action': 'tvshows&folderName=%s' % cleangenre.lang(i[0], self.lang)})
			...
			if self.simkl_link in url:
				self.list.append({'content': 'genres', 'name': cleangenre.lang(i[0], self.lang), 'url': url % i[1], 'image': '', 'icon': '', 'action': 'simklTvshows&foldername=%s' % i[0]})
			if 'trakt_tvshow_genre' in url:
				self.list.append({'content': 'genres', 'name': cleangenre.lang(i[0], self.lang), 'url': i[1], 'image': f"{i[0]}.jpg" if i[2] else '', 'icon': f"{i[0]}.png" if i[2] else '', 'action': 'trakt_tvshow_genre&mediatype=TVShows&genre=%s&folderName=%s' % (i[0], cleangenre.lang(i[0], self.lang))})
		self.addDirectory(self.list, folderName=folderName)
		return self.list
	...
	def trakt_person_list(self, url):
		self.list = []
		if ',return' in url: url = url.split(',return')[0]
		items = trakt.getTraktAsJson(url).get('cast')
		if not items: return
		q = dict(parse_qsl(urlsplit(url).query))
		index = int(q['page']) - 1
		total_pages = 1
		if getSetting('trakt.paginate.lists') == 'true':
			paginated_ids = [items[x:x + int(self.search_page_limit)] for x in range(0, len(items), int(self.search_page_limit))]
			total_pages = len(paginated_ids)
			items = paginated_ids[index]
		try:
			if int(q['limit']) != len(items): raise Exception()
			if int(q['page']) == total_pages: raise Exception()
			q.update({'page': str(int(q['page']) + 1)})
...
#################################
	...
	def tvshowDirectory(self, items, next=True, isProgress=False, isWatched=False, folderName='Infinity'):
		...
		nextMenu, findSimilarMenu, trailerMenu, exitMenu = getLS(32053), getLS(32184), getLS(40431), getLS(40477)
...
####-Context Menu and Overlays-####
				...
				try:
					...
					if watched:
						if isProgress and not self.watched_progress: continue
				...
				if favoriteItems and imdb in favoriteItems:
				...
				cm.append(('[COLOR FFE68B00]Infinity Settings[/COLOR]', 'RunPlugin(%s?action=tools_openSettings)' % sysaddon))
				if not is_widget: cm.append((exitMenu, 'Container.Refresh(%s?action=tvNavigator)' % sysaddon))
####################################
				...
				if 'castandart' in i: meta.update({"cast": ['castandart']})
				item.setArt(art)
				try: 
					...
					if count:
						if self.hideCounts and count['watched'] == count['total']: raise Exception()
						...
						if self.hideCounts: raise Exception()
						if KODI_VERSION >= 20: item.setProperties({'UnWatchedEpisodes': str(meta.get('total_aired_episodes', ''))}) # for shows never watched
						else: item.setProperties({'WatchedEpisodes': '0', 'UnWatchedEpisodes': str(meta.get('total_aired_episodes', ''))}) # for shows never watched
				...
				if is_widget: item.setProperty('isInfinity_widget', 'true')
				if is_widget:
					if self.hide_watched_in_widget and str(meta.get('playcount', 0)) == '1':
						if str(control.infoLabel("Window.Property(xmlfile)")) != 'Custom_1114_Search.xml':
							continue
				#item.setInfo(type='video', infoLabels=control.metadataClean(meta))
				control.set_info(item, meta, setUniqueIDs={'imdb': imdb, 'tmdb': tmdb, 'tvdb': tvdb})
				if is_widget and not self.useFullContext and KODI_VERSION > 19.5:
					pass
				else:
...
#				nextMenu = '[COLOR skyblue]' + nextMenu + page + '[/COLOR]'
				...
				u = urlparse(url).netloc.lower()
				if 'people/' in url:
					url = '%s?action=tvPersons&url=%s' % (sysaddon, quote_plus(url))
				elif u in self.imdb_link or u in self.trakt_link:
	...
	def addDirectory(self, items, queue=False, folderName='Infinity'):
		from sys import argv # some functions like ActivateWindow() throw invalid handle less this is imported here.
		control.playlist.clear()
		if not items: # with reuselanguageinvoker on an empty directory must be loaded, do not use sys.exit()
		...
		for i in items:
			try:
				...
				if content == 'genres':
					icon = control.joinPath(control.genreIconPath(), i['icon']) if i['icon'] else 'DefaultFolder.png'
					poster = control.joinPath(control.genrePosterPath(), i['image']) if i['image'] else addonThumb
				else:
					icon = i['icon']
...
#			nextMenu = '[COLOR skyblue]' + nextMenu + page + '[/COLOR]'
	...
	def traktBasedOn(self, url, create_directory=True, folderName=''):
		self.list = []
		try:
			link = 'https://api.trakt.tv/users/me/history/shows?limit=50'
			random_items = cache.get(self.trakt_list, 24, link, self.trakt_user, folderName)
			item = random.choice(random_items)
			try: url = getattr(self, url)
			except: pass
			if 'similar' in url: amble, prop = getLS(40259), 'infinity.tvsimilar'
			else: amble, prop = getLS(40257), 'infinity.tvrecent'
			try:
				amble = '%s %s' % (amble, item.get('title', ''))
				homeWindow.setProperty(prop, amble)
				folderName = quote_plus(amble)
			except: pass
			url = url % (item.get('tmdb'), '%s')
	...
	def traktProgress(self, url, mediatype='progress', create_directory=True, folderName=''):
		self.list = []
		try:
			progress = mediatype == 'progress'
			watched = mediatype == 'watched'
			try: url = getattr(self, url + '_link')
			except: pass
			self.list = cache.get(self.trakt_list, self.trakt_progress_hours, url, self.trakt_user, folderName)
			self.sort(type=mediatype)
			next = ''
			for i in range(len(self.list)): self.list[i]['next'] = next
			self.worker()
			if self.list is None: self.list = []
			...
			if create_directory: self.tvshowDirectory(self.list, next=False, isProgress=progress, isWatched=watched, folderName=folderName)
			return self.list
		except:
			from resources.lib.modules import log_utils
			log_utils.error()
	...
	def trakt_genre_list_decade(self, decade='', listType='', genre='', url='', folderName=''):
		...
		genreslug = url
#		decade = decade
		folderName = '%s (%s)' % (folderName, decade)
		decades = {
			'1930-1939': '1930-1939', '1940-1949': '1940-1949', '1950-1959': '1950-1959', '1960-1969': '1960-1969', '1970-1979': '1970-1979',
			'1980-1989': '1980-1989', '1990-1999': '1990-1999', '2000-2009': '2000-2009', '2010-2019': '2010-2019', '2020-2029': '2020-2024'
		}.get(decade, '2020-2024')
		if listType == 'trending':
