...
class Movies:
	def __init__(self, notifications=True):
		...
		self.last_day_of_month = calendar.monthrange(datetime.now().year, datetime.now().month)[1]
...
#		if not self.tmdb_key: self.tmdb_key = ''
		...
		self.tmdb_trendingday_link = 'https://api.themoviedb.org/3/trending/movie/day?api_key=%s&language=en-US&region=US&page=1'
		self.tmdb_trendingweek_link = 'https://api.themoviedb.org/3/trending/movie/week?api_key=%s&language=en-US&region=US&page=1'
		self.search_tmdb_link = 'https://api.themoviedb.org/3/search/movie?api_key=%s&language=en-US&query=%s&region=US&page=1'
...
#		self.persons_link = 'https://www.imdb.com/search/name/?count=100&name='
		...
		self.persons_link = 'https://api.trakt.tv/people/%s/movies?limit=%s&page=1' % ('%s', self.search_page_limit)
		...
		self.trakttrending_recent_link = 'https://api.trakt.tv/movies/trending?limit=%s&page=1&years=%s' % (self.page_limit, '%s-%s' % (self.date_time.year - 1, self.date_time.year) if self.date_time.month < 6 else str(self.date_time.year))
		...
		self.simkl_animeweek_link = 'https://api.simkl.com/anime/genres/all/movies/all-years/popular-this-week'
		self.simkl_animedate_link = 'https://api.simkl.com/anime/genres/all/movies/all-years/release-date'
		self.simkl_animebest_link = 'https://api.simkl.com/anime/best/watched?type=movies'
		self.simkl_animevote_link = 'https://api.simkl.com/anime/best/voted?type=movies'
		self.simkl_animegenre_link = 'https://api.simkl.com/anime/genres/%s/movies/all-years'
		self.simkltrendingweek_link = 'https://api.simkl.com/movies/trending/week?extended=tmdb'

		self.dvd_release_list_link = 'https://www.dvdsreleasedates.com/digital-releases/'
	...
	def search_new(self):
		...
		try:
			...
			dbcur.close() ; dbcon.close()
		if self.traktCredentials and getSetting('searchmovie.indexer') == '1':
			action, url = 'movies', self.search_link + quote_plus(q)
		else:
			action, url = 'tmdbmovies', self.search_tmdb_link % ('%s', quote_plus(q))
		control.closeAll()
...
#		url = self.persons_link + quote_plus(q)
		...
		control.closeAll()
		control.execute('ActivateWindow(Videos,plugin://plugin.video.infinity/?action=moviePersons&url=%s&folderName=%s,return)' % (quote_plus(url), quote_plus(q)))
...
#		self.addDirectory(self.list, folderName=folderName)
	...
	def genres(self, url, folderName=''):
		...
		if 'trakt' in url: genres = [
			...
			('Anime', 'anime', True), ('Comedy', 'comedy', True), ('Crime', 'crime', True), ('Documentary', 'documentary', True),
			('Donghua', 'donghua', False), ('Drama', 'drama', True), ('Family', 'family', True), ('Fantasy', 'fantasy', True),
			('History', 'history', True), ('Holiday', 'holiday', False), ('Horror', 'horror', True), ('Music', 'music', True),
			('Musical', 'musical', True), ('Mystery', 'mystery', True), ('Romance', 'romance', True),
			('Science Fiction', 'science-fiction', True), ('Sporting Event', 'sporting-event', False),
			('Superhero', 'superhero', False), ('Suspense', 'suspense', False), ('Thriller', 'thriller', True), ('War', 'war', True),
		...
		for i in genres:
			if self.imdb_link in url:
				for j in re.findall(r'date\[(\d+)\]', url): url = url.replace('date[%s]' % j, (self.date_time - timedelta(days=int(j))).strftime('%Y-%m-%d'))
			...
			if self.simkl_link in url:
				self.list.append({'content': 'genres', 'name': cleangenre.lang(i[0], self.lang), 'url': url % i[1], 'image': '', 'icon': '', 'action': 'simklMovies&foldername=%s' % i[0]})
			if 'trakt_movie_genre' in url:
				self.list.append({'content': 'genres', 'name': cleangenre.lang(i[0], self.lang), 'url': i[1], 'image': f"{i[0]}.jpg" if i[2] else '', 'icon': f"{i[0]}.png" if i[2] else '', 'action': 'trakt_movie_genre&mediatype=Movies&genre=%s&folderName=%s' % (i[0], cleangenre.lang(i[0], self.lang))})
		self.addDirectory(self.list, folderName=folderName)
		return self.list
...
#################################
	...
	def movieDirectory(self, items, unfinished=False, next=True, folderName=''):
		...
		useReleaseYear = getSetting('movies.showyear') == 'true'
		...
		nextMenu, clearSourcesMenu, exitMenu = getLS(32053), getLS(32611), getLS(40476)
		...
		for i in items:
			try:
				...
				meta.update({'poster': poster, 'fanart': fanart, 'banner': banner, 'clearlogo': clearlogo})
...
####-Context Menu and Overlays-####
				...
				if not is_widget: cm.append((exitMenu, 'Container.Refresh(%s?action=movieNavigator)' % sysaddon))
####################################
				...
				control.set_info(item, meta, setUniqueIDs={'imdb': imdb, 'tmdb': tmdb})
...
#				nextMenu = '[COLOR skyblue]' + nextMenu + page + '[/COLOR]'
				...
				u = urlparse(url).netloc.lower()
				if 'people/' in url: url = '%s?action=moviePersons&url=%s' % (sysaddon, quote_plus(url))
...
#			nextMenu = '[COLOR skyblue]' + nextMenu + page + '[/COLOR]'
	...
	def traktBasedOn(self, url, create_directory=True, folderName=''):
		self.list = []
		try:
			link = 'https://api.trakt.tv/users/me/history/movies?limit=50'
			randomItems = cache.get(self.trakt_list, 24, link, self.trakt_user, folderName)
			item = random.choice(randomItems)
			try: url = getattr(self, url)
			except: pass
			if 'similar' in url: amble, prop = getLS(40259), 'infinity.moviesimilar'
			else: amble, prop = getLS(40257), 'infinity.movierecent'
			try:
...
#		decade = decade
	...
	def getDvdReleaseList(self, url, create_directory=True, folderName=''):
		self.list = []
		try:
			try: url = getattr(self, url + '_link')
			except: pass
			result = cache.get(client.request, 96, url).replace('\n', ' ')
