...
#		self.persons_link = 'https://www.imdb.com/search/name/?count=100&name='
	...
	def search_new(self):
		...
		control.closeAll()
		control.execute('ActivateWindow(Videos,plugin://plugin.video.infinity/?action=movies&url=%s,return)' % (quote_plus(url)))
...
#		self.addDirectory(self.list, folderName=folderName)
	...
	def imdb_list(self, url, comingSoon=False, isRatinglink=False, folderName=''):
		from resources.lib.modules import log_utils
		log_utils.log('Infinity IMDB list generation. URL: %s'% str(url), 1)
		list = []
		try:
...
####-Context Menu and Overlays-####
				...
				if not is_widget and page > 2:
					cm.append((exitMenu, 'Container.Refresh(%s?action=movieNavigator)' % sysaddon))
	...
	def trakt_based_on_recent(self, create_directory=True, folderName=''):
		...
		try:
			...
			try: 
				...
				homeWindow.setProperty('infinity.movierecent', str(getLS(40257)+' '+item.get('title')))
			except: pass
			next = ''
			for i in range(len(self.list)): self.list[i]['next'] = next
			self.worker()
	...
	def trakt_based_on_similar(self, create_directory=True, folderName=''):
		...
		try:
			...
			try: 
				...
				homeWindow.setProperty('infinity.moviesimilar', str(getLS(40259)+' '+item.get('title')))
			except: pass
			next = ''
			for i in range(len(self.list)): self.list[i]['next'] = next
			self.worker()
...
#		decade = decade
	...
	def imdb_genre_list(self, url, comingSoon=False, isRatinglink=False, folderName=''):
			from resources.lib.modules import log_utils
			log_utils.log('Infinity Genre IMDB list generation. URL: %s'% str(url), 1)
			list = []
			try:
				for i in re.findall(r'date\[(\d+)\]', url):
					url = url.replace('date[%s]' % i, (self.date_time - timedelta(days=int(i))).strftime('%Y-%m-%d'))
	...
	def getDvdReleaseList(self, create_directory=True, folderName=''):
		self.list = cache.get(self.dvd_release_list, 96)
		self.worker()
		if self.list is None: self.list = []
		if create_directory: self.movieDirectory(self.list, folderName=folderName)

	def dvd_release_list(self):
		self.list = []
		try:
	...
	def recommendedFromLibrary(self, folderName=''):
		#from resources.lib.modules import log_utils
	...
	def similarFromLibrary(self, tmdb=None, create_directory=True, folderName=''):
		...
		try:
			...
			all_titles = list()
			if originalMovie:
				#log_utils.log('[plugin.video.infinity] Original Movie properties %s.'% str(originalMovie),1)
			...
			similar_score_dict = {}
			self.list = sorted(self.list, key=lambda k: k.get("similarscore"), reverse=True)
			...
			if originalMovie:
				try: 
					...
					homeWindow.setProperty('infinity.moviesimilarlibrary', str(getLS(40257)+' '+originalMovie["title"]))
				except: pass
			if self.list is None: self.list = []
			if create_directory: self.movieDirectory(self.list, folderName=folderName)
			return self.list
