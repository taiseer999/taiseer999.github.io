...
class Episodes:
		...
		try:
			if season is None and episode is None: # for "flatten" setting
				...
				try:
					...
					self.list = sorted(all_episodes, key=lambda k: (k['season'], k['episode']), reverse=getSetting('flatten.desc') == 'true')
				except: 
	...
	def episodeDirectory(self, items, unfinished=False, next=True, playlist=False, folderName=''):
		...
		trailerMenu, addToFavourites, removeFromFavourites = getLS(40431), getLS(40463), getLS(40468)
		...
		for i in items:
			try:
				...
				mediatype = 'tvshow' if isMultiList and multi_unwatchedEnabled else 'episode'
...
####-Context Menu and Overlays-####
				...
				if favoriteItems and f"{imdb}{season}{episode}" in favoriteItems:
...
####################################
					...
					if 'ForceAirEnabled' not in i:
						try:
							...
							if count:
								...
								if control.getKodiVersion() >= 20: item.setProperties({'UnWatchedEpisodes': str(meta.get('total_aired_episodes', ''))}) # for shows never watched
				...
				if  int(season) == 1 and meta.get('episode_type') == 'season_premiere':
					episode_type = 'series_premiere'
				elif (int(season) == meta.get('total_seasons')
					and meta.get('status') in ('Ended', 'Cancelled')
					and meta.get('episode_type') == 'season_finale'):
					episode_type = 'series_finale'
				else:
					episode_type = meta.get('episode_type')
				item.setProperty('episode_type', episode_type)
	...
	def addDirectory(self, items, queue=False, folderName=''):
		...
		for i in items:
			try:
				...
				url = 'plugin://plugin.video.infinity/?action=%s' % i['action']
