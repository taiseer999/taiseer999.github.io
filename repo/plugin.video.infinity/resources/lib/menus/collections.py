...
#		if self.tmdb_key == '' or self.tmdb_key is None: self.tmdb_key = ''
	...
	def search_new(self):
		...
		control.closeAll()
		control.execute('ActivateWindow(Videos,plugin://plugin.video.infinity/?action=collections&url=%s,return)' % (quote_plus(url)))
...
#				nextMenu = '[COLOR skyblue]' + nextMenu + page + '[/COLOR]'
	...
	def addDirectoryItem(self, name, action, poster, icon, fanart=None, context=None, queue=False, folderName=''):
		try:
			...
			url = 'plugin://plugin.video.infinity/?action=%s' % action
			cm = []
