		...
		try:
			if action in self.selection_actions:
				...
				if focus_id == 2040: # listItems
					...
					if chosen_listitem.getProperty('infinity.isHidden') == 'true':
						...
						if chosen_listitem.getProperty('infinity.isSelected') == '':
							chosen_listitem.setProperty('infinity.isSelected', 'true')
		...
		def builder():
			for count, item in enumerate(self.results, 1):
				try:
					...
					listitem.setProperty('infinity.tvshowtitle', item.get('tvshowtitle'))
					listitem.setProperty('infinity.year', str(item.get('year')))
					listitem.setProperty('infinity.isHidden', str(item.get('isHidden')))
					listitem.setProperty('infinity.isSelected', str(item.get('isHidden')))
					listitem.setProperty('infinity.imdb', item.get('imdb'))
					listitem.setProperty('infinity.tmdb', str(item.get('tmdb')))
					listitem.setProperty('infinity.tvdb', str(item.get('tvdb')))
					listitem.setProperty('infinity.status', item.get('status'))
					try: listitem.setProperty('infinity.watched_count', '(watched ' + str(item.get('watched_count').get('watched')) + ' of ' + str(item.get('watched_count').get('total')) + ')')
					except: pass
					...
					# listitem.setProperty('infinity.duration', str(item.get('duration'))) # not used
