...
class PlayNextXML(BaseDialog):
	...
	def onInit(self):
		playerWindow.setProperty('infinity.playnextPlayPressed', str(0))
		self.set_properties()
	...
	def set_properties(self):
		...
		try:
			...
			self.setProperty('infinity.tvshowtitle', self.meta.get('tvshowtitle'))
			...
			self.setProperty('infinity.title', self.meta.get('title'))
			self.setProperty('infinity.year', str(self.meta.get('year', '')))
			...
			self.setProperty('infinity.rating', str(self.meta.get('rating', '')))
			self.setProperty('infinity.landscape', self.meta.get('landscape', ''))
			self.setProperty('infinity.fanart', self.meta.get('fanart', ''))
			self.setProperty('infinity.thumb', self.meta.get('thumb', ''))
			self.setProperty('infinity.episode_type', self.meta.get('episode_type'))
			try:
				next_duration = int(self.meta.get('duration')) if self.meta.get('duration') else ''
				self.setProperty('infinity.duration', str(int(next_duration)))
				endtime = (datetime.now() + timedelta(seconds=next_duration)).strftime('%I:%M %p').lstrip('0') if next_duration else ''
				self.setProperty('infinity.endtime', endtime)
			except:
				self.setProperty('infinity.duration', '')
				self.setProperty('infinity.endtime', '')
			self.setProperty('infinity.playnext.background.color', self.playNextBG)
			if getSetting('playnext.hidebutton') == 'false':
				self.setProperty('infinity.hidebutton','true')
