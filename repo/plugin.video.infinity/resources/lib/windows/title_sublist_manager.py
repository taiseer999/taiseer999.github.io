...
class TitleSublistManagerXML(BaseDialog):
		...
		try:
			if action in self.selection_actions:
				...
				if focus_id == 2050: # listItems
					...
					org_id = chosen_listitem.getProperty('infinity.originalTitle')
					if chosen_listitem.getProperty('infinity.isSelected') == 'true':
						...
						original_title = chosen_listitem.getProperty('infinity.originalTitle')
						sub_title = chosen_listitem.getProperty('infinity.subTitle')
		...
		def builder():
			for count, item in enumerate(self.results, 1):
				try:
					...
					listitem.setProperty('infinity.originalTitle', item.get('originalTitle'))
					listitem.setProperty('infinity.subTitle', str(item.get('subTitle')))
	...
	def set_properties(self):
		try:
			...
			self.setProperty('infinity.infinity_icon', joinPath(artPath(), 'icon.png'))
		except:
			from resources.lib.modules import log_utils
			log_utils.error()
