...
class SourceResultsXML(BaseDialog):
		...
		def builder():
			for count, item in enumerate(self.results, 1):
				try:
					...
					extra_info = extra_info.replace('/', '').split('GB ', 1)[-1]
					#from resources.lib.modules import log_utils
					#log_utils.log('Infinity Sources Make Items: %s' % str(item.get('debrid')), log_utils.LOGINFO)
					...
					listitem.setProperty('infinity.quality_icon1', str(quality1_icon))
					yield listitem
				except:
					from resources.lib.modules import log_utils
					log_utils.error()
	...
	def set_properties(self):
		...
		try:
			...
			self.setProperty('infinity.dialog.color', self.dialogColor)
			self.setProperty('infinity.fanartdefault', addonFanart())
			...
			if getSetting('sources.backbutton') == 'true':
				self.setProperty('infinity.sourcebackbutton', '1')
			else:
				self.setProperty('infinity.sourcebackbutton', '0')
			if getSetting('sources.highlightmethod') == '1':
				self.setProperty('infinity.useprovidercolors', '1')
				self.setProperty('infinity.realdebridcolor', self.realdebridHighlightColor)
				self.setProperty('infinity.alldebridcolor', self.alldebridHighlightColor)
				self.setProperty('infinity.premiumizecolor', self.premiumizeHighlightColor)
				self.setProperty('infinity.plexcolor', self.plexHighlightColor)
				self.setProperty('infinity.easynewscolor', self.easynewsHighlightColor)
				self.setProperty('infinity.gdrivecolor', self.gdriveHighlightColor)
				self.setProperty('infinity.filepursuitcolor', self.filePursuitHighlightColor)
				
				if getSetting('sources.usecoloricons') == 'true':
					self.setProperty('infinity.usecoloricons', '1')
				else:
					self.setProperty('infinity.usecoloricons', '0')
			else:
				self.setProperty('infinity.useprovidercolors', '0')
				self.setProperty('infinity.usecoloricons', '0')
		except:
			from resources.lib.modules import log_utils
			log_utils.error()
