			...
			if action in self.selection_actions:
				...
				if focus_id == 2050: # listItems
					...
					for item in self.item_list:
						...
						self.selected_items.append(item.getProperty('infinity.trakt'))
				...
				if not source_trailer:
					...
					if media_type == 'show':
						...
						source_trailer = trailer.Trailer().worker('movie', chosen_listitem.getProperty('infinity.title'), chosen_listitem.getProperty('infinity.year'), None, chosen_listitem.getProperty('infinity.imdb'))
		...
		def builder():
			for count, item in enumerate(self.results, 1):
				try:
					...
					if item.get('tvshowtitle'): listitem.setProperty('infinity.media_type', 'show')
					else: listitem.setProperty('infinity.media_type', 'movie')
