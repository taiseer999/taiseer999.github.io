...
def traktAuth(fromSettings=0):
	...
	try:
		...
		if deviceCode:
			...
			if fromSettings == 1:
				control.openSettings('8.0', 'plugin.video.infinity')
			return True
		if fromSettings == 1:
				control.openSettings('8.0', 'plugin.video.infinity')
...
def getTraktDeviceToken(traktDeviceCode):
	...
	try:
		...
		if control.setting('dialogs.useinfinitydialog') == 'true':
			try:
				qr_url = '&color=f00&data=%s' % quote_plus(traktDeviceCode['verification_url'])
				qr_icon = 'https://api.qrserver.com/v1/create-qr-code/?size=256x256&qzone=1%s' % qr_url
			except: qr_icon = ''
			progressDialog = control.getProgressWindow(getLS(32073), icon=qr_icon)
...
def traktClientID():
	return getSetting('trakt.clientid')

def traktClientSecret():
	return getSetting('trakt.clientsecret')
