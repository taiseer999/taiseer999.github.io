...
def cache_clear_search():
	...
	try:
		...
		dbcur = dbcon.cursor()
		for t in ('movies', 'tvshow', 'collections', 'easynews'):
...
def cache_clear_thumbnails():
	...
	try:
		...
		dbcur = dbcon.cursor()
		icon = dbcur.execute("SELECT cachedurl FROM texture WHERE url ='" + addonPath + "/plugin.video.infinity/icon.png';").fetchone()
		fanart = dbcur.execute("SELECT cachedurl FROM texture WHERE url ='" + addonPath + "/plugin.video.infinity/fanart.png';").fetchone()
		if icon is not None:
			if xbmcvfs.exists(thumbnailsPath + icon[0]):
				xbmcvfs.delete(thumbnailsPath + icon[0])
			dbcur.execute("DELETE FROM texture WHERE url ='" + addonPath + "/plugin.video.infinity/icon.png';") #delete the icon
		if fanart is not None:
			if xbmcvfs.exists(thumbnailsPath + fanart[0]):
				xbmcvfs.delete(thumbnailsPath + fanart[0])
			dbcur.execute("DELETE FROM texture WHERE url ='" + addonPath + "/plugin.video.infinity/fanart.png';") #delete the fanart
		dbcon.close()
		cleared = True
	except:
		from resources.lib.modules import log_utils
...
def get_connection_bookmarks():
	...
	return conn
##################
def clear_local_bookmarks(): # clear all infinity bookmarks from kodi database
	try:
		dbcon = db.connect(get_video_database_path())
		dbcur = dbcon.cursor()
		dbcur.execute('''SELECT * FROM files WHERE strFilename LIKE "%plugin.video.infinity%"''')
...
#	database_path = control.absPath(control.joinPath(control.dataPath, '..', '..', 'Database', )) # doesn't work with mysql
	database_path = control.transPath('special://profile/Database')
	...
	elif kodi_version == 21: database_path = control.joinPath(database_path, 'MyVideos131.db')
...
def clrCache_version_update(clr_providers=False, clr_metacache=False, clr_cache=False, clr_search=False, clr_bookmarks=False):
	try:
		...
		control.log('[ plugin.video.infinity ]  Forced cache clear for version update complete.', 1)
	except:
		from resources.lib.modules import log_utils
		log_utils.error()
...
def update_cache_version():
	...
	try:
		...
		from resources.lib.modules import log_utils
		log_utils.log('Infinity Addon Data Path Does not Exist. Creating Folder....', __name__, level=log_utils.LOGDEBUG)
		ad_folder = control.transPath('special://profile/addon_data/plugin.video.infinity')
		control.makeDirs(ad_folder)
	try:
		with open(versionFile, 'r') as fh: oldVersion = fh.read()
	except: oldVersion = '0'
	try:
		curVersion = control.addon('plugin.video.infinity').getAddonInfo('version')
...
def get_cache_version():
	...
	try:
		...
		from resources.lib.modules import log_utils
		log_utils.log('Infinity Addon Data Path Does not Exist. Creating Folder....', __name__, log_utils.LOGDEBUG)
		ad_folder = control.transPath('special://profile/addon_data/plugin.video.infinity')
		control.makeDirs(ad_folder)
	try:
		with open(versionFile, 'r') as fh: oldVersion = fh.read()
