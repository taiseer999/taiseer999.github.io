...
class Premiumize:
	...
	def __init__(self):
		...
		self.headers = {'User-Agent': 'Infinity for Kodi', 'Authorization': 'Bearer %s' % self.token}
	...
	def auth(self, fromSettings=0):
		...
		if control.setting('dialogs.useinfinitydialog') == 'true':
			try:
				qr_url = '&data=%s' % quote_plus(token['verification_uri'])
				qr_icon = 'https://api.qrserver.com/v1/create-qr-code/?size=256x256&qzone=1%s' % qr_url
			except: qr_icon = ''
			self.progressDialog = control.getProgressWindow(getLS(40054), icon=qr_icon)
		else:
		...
		if success:
			if fromSettings == 1:
				control.openSettings('10.1', 'plugin.video.infinity')
	...
	def add_uncached_torrent(self, magnet_url, pack=False):
		...
		#####################################
		if control.setting('dialogs.useinfinitydialog') == 'true':
			self.progressDialog = control.getProgressWindow(getLS(40018), icon='')
