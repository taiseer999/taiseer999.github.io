...
class AllDebrid:
	...
	def auth(self, fromSettings=0):
		...
		if control.setting('dialogs.useinfinitydialog') == 'true':
			try:
				qr_url = '&data=%s' % quote_plus(response['user_url'])
				qr_icon = 'https://api.qrserver.com/v1/create-qr-code/?size=256x256&qzone=1%s' % qr_url
			except: qr_icon = ''
			self.progressDialog = control.getProgressWindow(getLS(40056), icon=qr_icon)
		else:
		...
		if self.token in (None, '', 'failed'):
			if fromSettings == 1:
				control.openSettings('10.0', 'plugin.video.infinity')
			return
		control.sleep(2000)
