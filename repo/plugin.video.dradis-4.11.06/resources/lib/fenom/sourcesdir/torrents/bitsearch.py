...
from resources.lib.fenom import source_utils, log_utils
...
# (1=other/video, 2=movies, 3=TV) but seem to produce bogus results, do not use
		self.search_link = '/search?q=%s&limit=50&sort=size' if int(getSetting('bitsearch.sort')) else '/search?q=%s&limit=50'
		self.min_seeders = 0
