...
#		self.addDirectoryItem(f"{getLS(32002)} (IMDB)" if self.indexLabels else 32002, 'anime_TVshows&url=anime', 'tvshows.png', 'DefaultTVShows.png')
		...
		self.addDirectoryItem('Series Popular This Week (SIMKL)' if self.indexLabels else 'Series Popular This Week', 'tvshows&url=simkl_animeweek', 'tvshows.png', 'DefaultTVShows.png')
		self.addDirectoryItem('Series Top Ranked (SIMKL)' if self.indexLabels else 'Series Top Ranked', 'tvshows&url=simkl_animerank', 'tvshows.png', 'DefaultTVShows.png')
		self.addDirectoryItem('Series Recently Released (SIMKL)' if self.indexLabels else 'Series Recently Released', 'tvshows&url=simkl_animedate', 'tvshows.png', 'DefaultTVShows.png')
		...
		self.addDirectoryItem('Movies Popular This Week (SIMKL)' if self.indexLabels else 'Movies Popular This Week', 'movies&url=simkl_animeweek', 'movies.png', 'DefaultMovies.png')
		self.addDirectoryItem('Movies Top Ranked (SIMKL)' if self.indexLabels else 'Movies Top Ranked', 'movies&url=simkl_animerank', 'movies.png', 'DefaultMovies.png')
		self.addDirectoryItem('Movies Recently Released (SIMKL)' if self.indexLabels else 'Movies Recently Released', 'movies&url=simkl_animedate', 'movies.png', 'DefaultMovies.png')
		self.endDirectory()
...
#		self.addDirectoryItem(32531, 'tools_ShowChangelog&name=FenomScrapers', 'tools.png', 'DefaultAddonProgram.png', isFolder=False)
	...
	def alldebrid_service(self):
		if getSetting('alldebrid.token'):
			...
			self.addDirectoryItem('[I]Please visit My Accounts for setup[/I]', 'tools_openSettings&query=6.0', 'alldebrid.png', 'DefaultAddonService.png', isFolder=False)
		self.endDirectory()
	...
	def easynews_service(self):
		if getSetting('easynews.username'):
			...
			self.addDirectoryItem('[I]Please visit My Accounts for setup[/I]', 'tools_openSettings&query=6.0', 'easynews.png', 'DefaultAddonService.png', isFolder=False)
		self.endDirectory()
	...
	def realdebrid_service(self):
		if getSetting('realdebrid.token'):
			...
			self.addDirectoryItem('[I]Please visit My Accounts for setup[/I]', 'tools_openSettings&query=6.0', 'realdebrid.png', 'DefaultAddonService.png', isFolder=False)
		self.endDirectory()
	...
	def views(self):
		try:
			...
			title = getLS(32059)
			url = 'plugin://plugin.video.dradis/?action=tools_addView&content=%s' % content
