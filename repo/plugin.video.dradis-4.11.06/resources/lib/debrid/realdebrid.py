...
#				url += "&auth_token=%s" % self.token
	...
	def auth(self):
		...
		try:
			qr_url = '&data=%s' % quote_plus(response['verification_url'])
		...
		while self.secret == '':
			...
			control.sleep(1000)
			time_passed -= 1
			progressDialog.update(int(time_passed / self.auth_timeout * 100))
			if not time_passed % self.auth_step: self.auth_loop()
...
#					isFolder = True if item['status'] == 'downloaded' else False
					isFolder = True if item.get('ended') else False
...
#				nextMenu = '[COLOR skyblue]' + nextMenu + page + '[/COLOR]'
				highlight_color = control.getHighlightColor() ; nextMenu = f"[COLOR {highlight_color}]{nextMenu}{page}[/COLOR]"
