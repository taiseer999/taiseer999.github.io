...
V2_API_KEY = getSetting('trakt.client_id')
...
def unHideItems(tvdb_ids):
	...
	try:
		...
		for section in sections:
			...
			control.sleep(1000)
		if success:
			if 'plugin.video.dradis' in control.infoLabel('Container.PluginName'): control.refresh()
	...
	try:
		...
		if select >= 0:
			if items[select][1] == 'watch':
				...
				control.execute('RunPlugin(plugin://plugin.video.dradis/?action=shows_traktHiddenManager)')
			elif items[select][1] == 'unfinishedEpisodeManager':
				control.execute('RunPlugin(plugin://plugin.video.dradis/?action=episodes_traktUnfinishedManager)')
			elif items[select][1] == 'unfinishedMovieManager':
				control.execute('RunPlugin(plugin://plugin.video.dradis/?action=movies_traktUnfinishedManager)')
				...
				if items[select][0].startswith(('Add', 'Rem')): cache.cache_clear(flush_only=True)
...
#	if not control.yesnoDialog(getLS(32056), '', ''): return
	...
	funcs = [sync_playbackProgress, sync_watchedProgress, sync_watched, sync_hidden_progress, sync_liked_lists, sync_collection, sync_watch_list, sync_user_lists]
	threads = [Thread(target=i, kwargs={'forced': True}) for i in funcs]
...
def auth():
	response = getTraktAsJson('oauth/device/code', {'client_id': V2_API_KEY})
	device_code = response['device_code']
	expires_in = int(response['expires_in'])
	interval = int(response['interval'])
	data = {'code': device_code, 'client_id': V2_API_KEY, 'client_secret': CLIENT_SECRET}
	try:
		qr_url = '&color=f00&data=%s' % quote_plus(response['verification_url'])
		qr_icon = 'https://api.qrserver.com/v1/create-qr-code/?size=256x256&qzone=1%s' % qr_url
		control.notification(message=response['verification_url'], icon=qr_icon, time=15000)
	except: pass
	verification_url = getLS(32513) % (highlight_color, response['verification_url'])
	user_code = getLS(32514) % (highlight_color, response['user_code'])
	dialog_text = '%s[CR]%s' % (verification_url, user_code)
	progressDialog = control.progressDialog
	progressDialog.create(getLS(32073), dialog_text)
	token = ''
	...
	while not token and not progressDialog.iscanceled() and time_passed:
		...
		control.sleep(1000)
		time_passed -= 1
		if time_passed % interval: continue
		response = getTrakt('oauth/device/token', post=data)
		if not response: continue
		try: token = response.json()
		except: control.notification(message='Trakt authorization failed.')
	...
	if token:
		...
		response = getTraktAsJson('users/me')
		setSetting('trakt.username', str(response['username']))
		setSetting('trakt.token', token['access_token'])
		setSetting('trakt.refresh', token['refresh_token'])
		setSetting('trakt.expires', str(expires))
		setSetting('trakt.isauthed', 'true')
...
def deauth():
	data = {'token': getSetting('trakt.token'), 'client_id': V2_API_KEY, 'client_secret': CLIENT_SECRET}
	response = getTrakt('oauth/revoke', data)
	setSetting('trakt.username', '')
	setSetting('trakt.expires', '')
	setSetting('trakt.refresh', '')
	setSetting('trakt.token', '')
	setSetting('trakt.isauthed', '')
	control.dialog.ok('Trakt', getLS(32320))
