...
from modules import kodi_utils, settings, metadata, tracking
...
progress_db_string, indicators_dict = 'fenlight_hidden_progress_items', {0: 'watched_db', 1: 'trakt_db', 2: 'simkl_db'}
	...
	try:
		...
		if watched_indicators:
			try:
				...
				sleep(1000)
				tracking.scrobble('clear_progress', media_type, media_id, 0, season, episode, resume_id)
			except: pass
			...
			def _process():
				for i in insert_list:
					try:
						...
						sleep(1000)
						tracking.scrobble('clear_progress', i[0], i[1], 0, i[2], i[3], resume_id)
					except: pass
...
def set_bookmark(params):
	try:
		...
		if watched_indicators:
			...
			else: tracking.scrobble('set_progress', media_type, tmdb_id, resume_point, season, episode, refresh_tracker=False)
		else:
...
def mark_movie(params):
	...
	watched_indicators = watched_indicators_function()
	if watched_indicators:
		if from_playback == 'true' and tracking.official_status(media_type) == False: sleep(1000)
