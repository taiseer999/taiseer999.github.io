...
def playback_choice(params):
	...
	items = [{'line': 'Select Source', 'function': 'scrape'},
			...
			{'line': 'Clear Debrid Cache & Show Results', 'function': 'clear_debrid_cache_and_show'},
	...
	check_cache_status, check_cache_toggle =  ('On', 'false') if settings.external_cache_check() else ('Off', 'true')
	items.append({'line': 'Toggle External Cache Check (Currently [B]%s[/B])' % check_cache_status, 'function': 'toggle_check_cache'})
	...
	if choice == 'scrape':
		...
		if disable_filters_choice:
			...
			kodi_utils.set_property('fs_filterless_search', 'true')
	elif choice == 'toggle_check_cache':
		set_setting('external.cache_check', check_cache_toggle)
		return kodi_utils.notification('External Cache Check set to %s' % ('ON' if check_cache_toggle == 'true' else 'OFF'), 2500)
