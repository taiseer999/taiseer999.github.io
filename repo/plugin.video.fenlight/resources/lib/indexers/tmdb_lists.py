...
def get_tmdb_lists(params):
	...
	def _process():
		for item in data:
			...
			# ('[B]Edit Properties[/B]', 'RunPlugin(%s)' % build_url({'mode': 'tmdblist.adjust_tmdb_list_properties',
			# 	'list_id': list_id, 'original_list_name': list_name, 'custom_poster': custom_poster, 'custom_fanart': custom_fanart})),
...
# 		if list_name == None: return adjust_tmdb_list_properties(params)
# 		current_name = list_name
# 		params.update({'list_name': current_name, 'refresh': 'true'})
# 	elif action == 'make_poster':
# 		new_poster = tmdb_image_maker(current_name, list_id, 'poster', custom_poster)
...
# 		new_fanart = tmdb_image_maker(current_name, list_id, 'fanart', custom_fanart)
...
# def tmdb_image_maker(list_name, list_id, image_type, custom_image):
...
# 	images = []
# 	for item in content:
# 		if item['media_type'] == 'movie': function = metadata.movie_meta
# 		else: function = metadata.tvshow_meta
# 		meta = function('tmdb_id', item['id'], tmdb_api_key(), mpaa_region(), get_datetime(), get_current_timestamp())
# 		if meta[image_type]: images.append(meta[image_type])
