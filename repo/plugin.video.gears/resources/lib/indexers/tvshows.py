...
class TVShows:
	...
	def fetch_list(self):
		...
		try:
			...
			if page_no == 1 and not self.is_external:
				...
				if not any([x in folder_path for x in ('build_season_list', 'build_episode_list')]): kodi_utils.set_property('gears.exit_params', folder_path)
	...
	def build_tvshow_content(self, _position, _id):
		try:
			...
			set_properties({
				'gears.extras_params': extras_params,
				'gears.options_params': options_params,
				'gears.browse_recommended_params': browse_recommended_params,
				'gears.browse_more_like_this_params': browse_more_like_this_params,
				'gears.browse_similar_params': browse_similar_params,
				'gears.browse_in_trakt_list_params': browse_in_trakt_list_params,
				'gears.trakt_manager_params': trakt_manager_params,
				'gears.personal_manager_params': personal_manager_params,
				'gears.tmdb_manager_params': tmdb_manager_params,
				'gears.favorites_manager_params': favorites_manager_params
				})
