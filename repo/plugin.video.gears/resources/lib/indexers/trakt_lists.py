...
from modules.utils import paginate_list, gen_md5, get_datetime, get_current_timestamp, sort_list
from modules.settings import paginate, page_limit, widget_hide_next_page, tmdb_api_key, mpaa_region, ignore_articles, trakt_lists_auto_load_az, trakt_lists_auto_load_max
		...
		if data:
			if shuffle_lists:
				...
				if returning_to_list:
					try: data = json.loads(kodi_utils.get_property('gears.trakt.lists.order'))
					except: pass
				else:
					shuffle(data)
					kodi_utils.set_property('gears.trakt.lists.order', json.dumps(data))
			else:
				kodi_utils.clear_property('gears.trakt.lists.order')
...
def build_trakt_list(params):
	...
	try:
		...
		if page_no == 1 and not is_external: kodi_utils.set_property('gears.exit_params', kodi_utils.folder_path())
		...
		# NEW (2.0.8): Auto-load Trakt list contents A-Z up to a configurable max.
		# Skips client-side pagination entirely so the whole list is presented as one
		# A-Z page (article-stripped sort), but capped so we don't stall on huge lists.
		auto_az = (not use_result) and trakt_lists_auto_load_az() and page_no == 1
		if auto_az and isinstance(result, list) and result:
			try: result = sort_list('title', 'asc', result, ignore_articles())
			except: pass
			max_items = trakt_lists_auto_load_max()
			if max_items > 0: result = result[:max_items]
			process_list, total_pages, paginate_start = result, 1, 0
		else:
		...
		# When auto A-Z is on, sort the Kodi row tuples by visible label (article-stripped)
		# so the final on-screen order matches user expectation across mixed media types.
		if auto_az:
			import re as _re
			def _az_key(row):
				try: label = row[1].getLabel()
				except: label = ''
				label = (label or '').lower()
				return _re.sub(r'^(the |a |an )', '', label) if ignore_articles() else label
			item_list.sort(key=_az_key)
		else: item_list.sort(key=lambda k: k[1])
