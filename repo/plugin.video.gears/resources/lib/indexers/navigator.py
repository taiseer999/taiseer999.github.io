...
class Navigator:
	def __init__(self, params):
		...
		self.category_name = self.params_get('name', 'gears')
	...
	def tools(self):
		...
		if get_setting('gears.external_scraper.module') not in ('empty_setting', ''):
		...
		if get_setting('gears.use_viewtypes', 'true') == 'true' and not get_setting('gears.manual_viewtypes', 'false') == 'true':
	...
	def build_shortcut_folder_contents(self):
		...
		if contents:
			for count, item in enumerate(contents):
				...
				icon = iconImage
				if iconImage:
					if iconImage.startswith('http') or 'plugin.video.gears' in iconImage: original_image = True
	...
	def exit_media_menu(self):
		params = k.get_property('gears.exit_params')
		if params: return k.container_refresh_input(params)

	def tips(self):
		tips_location = 'special://home/addons/plugin.video.gears/resources/text/tips'
