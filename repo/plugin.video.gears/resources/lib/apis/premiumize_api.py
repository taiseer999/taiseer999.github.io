...
class PremiumizeAPI:
	def __init__(self):
		self.token = get_setting('gears.pm.token', 'empty_setting')

	def auth(self):
		self.token = ''
		line = '%s[CR]%s[CR]%s'
		data = {'response_type': 'device_code', 'client_id': '522962560'}
		...
		data = {'grant_type': 'device_code', 'client_id': '522962560', 'code': device_code}
	...
	def headers(self):
		return {'User-Agent': 'gears for Kodi', 'Authorization': 'Bearer %s' % self.token}
