...
def addon_info(info):
	return xbmcaddon.Addon('plugin.video.gears').getAddonInfo(info)

def addon_version():
	return get_property('gears.addon_version') or addon_info('version')

def addon_path():
	return get_property('gears.addon_path') or addon_info('path')

def addon_profile():
	return get_property('gears.addon_profile') or translate_path(addon_info('profile'))

def addon_icon():
	return get_property('gears.addon_icon') or translate_path(addon_info('icon'))

def addon_icon_mini():
	return get_property('gears.addon_icon_mini') or os.path.join(addon_info('path'), 'resources', 'media', 'addon_icons', 'minis',
														os.path.basename(translate_path(addon_info('icon'))))

def addon_fanart():
	return get_property('gears.addon_fanart') or translate_path(addon_info('fanart'))

def get_icon(image_name, image_folder='icons'):
	return translate_path('special://home/addons/plugin.video.gears/resources/media/%s/%s.png' % (image_folder, image_name))

def get_addon_fanart():
	return get_property('gears.default_addon_fanart') or addon_fanart()

def build_url(url_params):
	return 'plugin://plugin.video.gears/?%s' % urlencode(url_params)
...
def set_view_mode(view_type, content='files', is_external=None):
	if not get_property('gears.use_viewtypes') == 'true': return
	if is_external == None: is_external = external()
	if is_external: return
	view_id = get_property('gears.%s' % view_type) or None
...
def logger(heading, function):
	...
	try:
		debug_on = kodi_window().getProperty('gears.addon_debug')
		if not debug_on:
			try:
				from caches.settings_cache import get_setting as _gs
				debug_on = _gs('gears.addon_debug')
			except: pass
		if debug_on != 'true': return
		import datetime
		log_file = xbmcvfs.translatePath('special://logpath/gears.log')
		now = datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')
		with open(log_file, 'a', encoding='utf-8') as f:
			f.write('%s [%s]: %s\n' % (now, heading, function))
	except Exception as e:
		xbmc.log('###gears_logger_error###: %s' % e, 1)

def clear_gears_log():
	try:
		log_file = xbmcvfs.translatePath('special://logpath/gears.log')
		open(log_file, 'w', encoding='utf-8').close()
		ok_dialog(heading='Gears Debug Log', text='Debug log cleared.')
	except: pass
...
def addon(addon_id='plugin.video.gears'):
...
def run_addon(addon='plugin.video.gears', block=False):
	return execute_builtin('RunAddon(%s)' % addon, block)

def external():
	return 'gears' not in get_infolabel('Container.PluginName')
...
def disable_enable_addon(addon_name='plugin.video.gears'):
	import json
	try:
...
def update_kodi_addons_db(addon_name='plugin.video.gears'):
	import time
...
def external_scraper_settings():
	try:
		external = get_property('gears.external_scraper.module')
...
def notification(line1, time=5000, icon=None):
	kodi_dialog().notification('gears', line1, icon or addon_icon(), time)
...
def volume_checker():
	# 0% == -60db, 100% == 0db
	try:
		if get_property('gears.playback.volumecheck_enabled') == 'false' or get_visibility('Player.Muted'): return
		from modules.utils import string_alphanum_to_num
		max_volume = min(int(get_property('gears.playback.volumecheck_percent') or '50'), 100)
...
def get_all_icon_vars():
	icon_items = list_dirs(translate_path('special://home/addons/plugin.video.gears/resources/media/icons'))[1]
