...
def sys_exit_check():
        if get_property('gears.reuse_language_invoker') == 'false': return False
        return external()
...
def routing(sys):
        ...
        if 'navigator.' in mode:
                ...
                _m = mode.split('.')[1]
                if _m.isidentifier() and not _m.startswith('_'): getattr(Navigator(params), _m)()
        elif 'menu_editor.' in mode:
                from modules.menu_editor import MenuEditor
                _m = mode.split('.')[1]
                if _m.isidentifier() and not _m.startswith('_'): getattr(MenuEditor(params), _m)()
        elif 'personal_lists.' in mode:
                from indexers import personal_lists
                _m = mode.split('.')[1]
                if _m.isidentifier() and not _m.startswith('_'): getattr(personal_lists, _m)(params)
        elif 'tmdblist.' in mode:
                from indexers import tmdb_lists
                _m = mode.split('.')[1]
                if _m.isidentifier() and not _m.startswith('_'): getattr(tmdb_lists, _m)(params)
        elif 'easynews.' in mode:
                from indexers import easynews
                _m = mode.split('.')[1]
                if _m.isidentifier() and not _m.startswith('_'): getattr(easynews, _m)(params)
        elif 'playback.' in mode:
                if mode == 'playback.meta':
                        from modules.sources import Sources
                        Sources().playback_prep(params)
                elif mode == 'playback.video':
                        from modules.player import gearsPlayer
                        gearsPlayer().run(_get('url', None), _get('obj', None))
        elif 'choice' in mode:
                from indexers import dialogs
                _m = mode
                if _m.isidentifier() and not _m.startswith('_'): getattr(dialogs, _m)(params)
        elif 'custom_key.' in mode:
                from modules import custom_keys
                _m = mode.split('custom_key.')[1]
                if _m.isidentifier() and not _m.startswith('_'): getattr(custom_keys, _m)()
        elif 'trakt.' in mode:
                if '.list' in mode:
                        from indexers import trakt_lists
                        _m = mode.split('.')[2]
                        if _m.isidentifier() and not _m.startswith('_'): getattr(trakt_lists, _m)(params)
                else:
                        from apis import trakt_api
                        _m = mode.split('.')[1]
                        if _m.isidentifier() and not _m.startswith('_'): getattr(trakt_api, _m)(params)
        elif 'build' in mode:
                ...
                _m = mode.split('.')[1]
                if _m.isidentifier() and not _m.startswith('_'): getattr(settings_cache, _m)(params)
        elif 'downloader.' in mode:
                from modules import downloader
                _m = mode.split('.')[1]
                if _m.isidentifier() and not _m.startswith('_'): getattr(downloader, _m)(params)
        elif 'updater' in mode:
                from modules import updater
                _m = mode.split('.')[1]
                if _m.isidentifier() and not _m.startswith('_'): getattr(updater, _m)()
        ##EXTRA modes##
        elif mode == 'set_view':
        ...
        elif mode == 'clear_gears_log':
                from modules.kodi_utils import clear_gears_log
                clear_gears_log()
